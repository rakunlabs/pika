...
{#if canRead}
 <div class="flex h-full w-full overflow-hidden bg-slate-100 dark:bg-warm-900">
...
{:else}
 ...
 <div class="flex h-full w-full items-center justify-center bg-slate-100 dark:bg-warm-900 px-4">
 ...
 <h1 class="text-5xl font-bold tracking-wide text-slate-800 dark:text-slate-100">pika</h1>
 {#if info?.subtitle}
 <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{info.subtitle}</p>
 {/if}
 </div>
 </div>
 {#if info?.version}
 <p class="mt-3 text-xs font-mono text-slate-400 dark:text-slate-500 select-text">{info.version}</p>
 {/if}
 <p class="mt-8 text-sm text-slate-500 dark:text-slate-400 text-center">
 Your account doesn't have access to any resources yet. Contact your administrator to request permissions.
 </p>
 </div>
 </div>
{/if}
