...
<div
    class="flex flex-col items-center justify-start h-full w-full bg-slate-100 dark:bg-warm-900 pt-8"
>
    <div class="w-full max-w-sm">
        <div
            class="relative bg-white dark:bg-warm-800 rounded-lg shadow-lg border border-slate-200 dark:border-warm-700 p-8"
        >
 ...
 a version — independent of whether `subtitle` is set. -->
            {#if appStore.info?.version}
                <span
                    class="absolute top-3 left-3 text-[10px] font-mono text-slate-400 dark:text-slate-500"
                >
 ...
 after login so the toggle is always available. -->
            ...
            <div class="flex items-center justify-center gap-3 mb-6">
                ...
                <div class="flex flex-col leading-tight">
                    <span
                        class="text-2xl font-semibold tracking-tight text-slate-800 dark:text-slate-100"
                    >
 ...
 spot — version lives in the top-left now. -->
                    <span
                        class="text-sm text-slate-500 dark:text-slate-400 min-h-5"
                    >
                        {loginInfo?.subtitle ?? ""}
                    </span>
                </div>
            ...
            {#if infoLoading}
                <div
                    class="text-center text-sm text-slate-400 dark:text-slate-500 py-4"
                >
                    Loading...
                </div>
            {:else if infoError}
                <div
                    class="p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-md text-sm text-red-700 dark:text-red-300"
                >
                    {infoError}
                </div>
            {:else if mfaChallenge}
                ...
                >
                    <div class="flex flex-col items-center text-center">
                        <ShieldCheck
                            size={32}
                            class="text-accent-600 dark:text-accent-400 mb-2"
                        />
                        <h2
                        ...
                        <p
                            class="mt-1 text-xs text-slate-500 dark:text-slate-400 max-w-xs"
                        >
                            Enter the 6-digit code from your authenticator app,
                            or one of your recovery codes if you lost the
                            device.
                        </p>
                    </div>
                    ...
                    {:else}
                        ...
                        >
                            ...
                            >
                                ...
                                {loading
                                    ? "Signing in..."
                                    : passwordStrategy.label || "Sign in"}
                            </button>
                ...
                {#if oauthStrategies.length > 0}
                    {#if passwordStrategy}
                        <div class="flex items-center gap-3 my-5">
                            <div
                                class="flex-1 h-px bg-slate-200 dark:bg-warm-900"
                            ></div>
                            <span
  ...
  affordance, which would confuse users on managed devices. -->
                {#if passkeyStrategy}
                    ...
                    >
                        ...
                        {loading
                            ? "Waiting for device..."
                            : passkeyStrategy.label || "Sign in with passkey"}
                    </button>
                {/if}
