<script lang="ts">
  ...
  // ── Tab state ──
  let activeSection = $state<'tokens' | 'external' | 'raw_mounts' | 'ftp_shares' | 'file_servers' | 'public_server' | 'hooks' | 'rotation' | 'security' | 'backup'>('tokens');
...
<div class="flex h-full overflow-hidden">
  ...
  <div class="w-52 shrink-0 bg-slate-50 border-r border-slate-200 overflow-y-auto">
    ...
    <nav class="flex flex-col gap-0.5 px-2 pb-4">
      ...
      <button
        class="flex items-center gap-2.5 w-full px-3 py-2 text-[13px] font-medium rounded-md cursor-pointer transition-colors text-left
          {activeSection === 'public_server' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-transparent text-slate-600 border border-transparent hover:bg-slate-100 hover:text-slate-800'}"
        onclick={() => { activeSection = 'public_server'; loadPublicServerSettings(); }}
      >
        <Globe size={15} class="shrink-0" />
        Public Server
      </button>
      <button
        class="flex items-center gap-2.5 w-full px-3 py-2 text-[13px] font-medium rounded-md cursor-pointer transition-colors text-left
          {activeSection === 'hooks' ? 'bg-blue-50 text-blue-700 border border-blue-200' : 'bg-transparent text-slate-600 border border-transparent hover:bg-slate-100 hover:text-slate-800'}"
        onclick={() => { activeSection = 'hooks'; hooks = [...(configStore.settings?.hooks || [])]; }}
      >
        <Webhook size={15} class="shrink-0" />
        Hooks
      </button>
      <button
        class="flex items-center gap-2.5 w-full px-3 py-2 text-[13px] font-medium rounded-md cursor-pointer transition-colors text-left
              ...
              <div class="col-span-2">
                ...
                {:else}
                  <div class="flex justify-end mb-2">
                    <button
                      class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        {#if sftpServeEnabled}
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="col-span-2">
              ...
              {:else}
                <div class="flex items-center justify-between mb-1.5">
                  <label for="sftp-host-key-pem" class="block text-xs font-medium text-slate-500">Host Key (PEM)</label>
                  <button
                    class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                {/if}
                <p class="mt-1 text-[11px] text-slate-400">Paste the SSH private key content directly, or click <strong>Generate Ed25519 Key</strong> to create one. Stored in the database.</p>
              {/if}
  ...
  <!-- ══════════════════════════════════════════ -->
  {#if activeSection === 'public_server'}
    <div>
      <div class="mb-6">
