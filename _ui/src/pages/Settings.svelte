<script lang="ts">
 import { Key, Globe, FolderOpen, Share2, Server, Webhook, RotateCw, Lock, HardDrive, ShieldCheck, Info, Users, Palette, KeyRound } from "lucide-svelte";
 ...
 type Section = 'appearance' | 'account_security' | 'tokens' | 'external' | 'raw_mounts' | 'ftp_shares' | 'file_servers' | 'public_server' | 'hooks' | 'auth' | 'user_sync' | 'rotation' | 'security' | 'backup' | 'about';

 // Section → required capability. `null` means always visible (e.g.
 // About, Appearance, Account Security — these are per-user and
 // intentionally require no capability).
