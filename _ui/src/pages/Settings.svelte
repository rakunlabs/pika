<script lang="ts">
  ...
  import type { TokenScope, CreateTokenRequest, ExternalResource, RawMountEntry, FTPShareEntry, FTPUserEntry, FTPServeSettings, SFTPServeSettings, TFTPServeSettings, Hook, HookTarget, KafkaSecurity, KafkaSASLEntry } from "@/lib/types/config";
  import { appStore } from "@/lib/store/store.svelte";
  import axios from 'axios';

  // ── Tab state ──
  let activeSection = $state<'tokens' | 'external' | 'raw_mounts' | 'ftp_shares' | 'file_servers' | 'hooks' | 'rotation' | 'security' | 'backup'>('tokens');
