...
<div class="h-full overflow-auto p-6">
  ...
  <div class="py-12 text-center text-sm text-slate-400 dark:text-warm-400">Loading…</div>
  {:else if !canManageUsers && !canManagePermissions}
  <!-- No access to either tab — show a stub instead of empty UI. -->
  <div class="bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-lg p-8 text-center">
  <Shield class="mx-auto text-slate-400 dark:text-slate-500" size={32} />
  <p class="mt-3 text-sm font-medium text-slate-700 dark:text-slate-200">No access</p>
  <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">You do not have permission to manage users or permissions.</p>
  </div>
  {:else}
 <!-- Header with tabs -->
 <div class="flex items-end justify-between mb-4 border-b border-slate-200 dark:border-warm-700">
 <div class="flex items-center gap-1">
  {#if canManageUsers}
  <button
  onclick={() => { activeTab = 'users'; }}
  class="px-4 py-2 text-sm font-medium transition-colors cursor-pointer border-b-2 -mb-px {activeTab === 'users'
  ...
  class="px-4 py-2 text-sm font-medium transition-colors cursor-pointer border-b-2 -mb-px {activeTab === 'permissions'
    ? 'border-accent-500 text-accent-700 dark:text-accent-300'
    : 'border-transparent text-slate-500 dark:text-warm-300 hover:text-slate-800 dark:hover:text-white hover:border-slate-300 dark:hover:border-warm-600'}"
  >
  Permissions
 ...
 <!-- Create User Form -->
 {#if showCreateForm}
 <div class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-4 mb-4">
 <h3 class="text-sm font-medium text-slate-700 dark:text-slate-200 mb-3">Create New User</h3>
 <div class="flex gap-3 items-end">
 <div class="flex-1">
 <label for="new-username" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Username</label>
 ...
 <div class="flex-1">
 <label for="new-password" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Password</label>
 ...
 <div class="relative">
 <Shield size={14} class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 pointer-events-none" />
 <select
 ...
 <ChevronDown size={14} class="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 pointer-events-none" />
 </div>
 {/if}
 </div>

 <!-- Users Table -->
 <div class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 overflow-hidden">
 <table class="w-full text-sm">
 <thead>
 <tr class="bg-slate-50 dark:bg-warm-800 border-b border-slate-200 dark:border-warm-700">
 <th class="text-left px-4 py-2.5">
 <button onclick={() => handleSort('username')} class="flex items-center gap-1 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider hover:text-slate-800 dark:hover:text-slate-100 transition-colors">
 Username
 ...
 <th class="text-left px-4 py-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
 <th class="text-left px-4 py-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Sessions</th>
 <th class="text-left px-4 py-2.5">
 <button onclick={() => handleSort('created_at')} class="flex items-center gap-1 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider hover:text-slate-800 dark:hover:text-slate-100 transition-colors">
 Created
 ...
 <th class="text-right px-4 py-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Actions</th>
 </tr>
 </thead>
 <tbody>
 ...
 {@const isOnline = user.active_sessions > 0}
  <tr class="border-b border-slate-100 dark:border-warm-700 {isYou
 ...
 {/if}
  ...
  <span class="font-medium {isYou ? 'text-accent-800 dark:text-accent-200' : 'text-slate-800 dark:text-slate-100'}">{user.username}</span>
  {#if isYou}
 ...
 <td class="px-4 py-3">
  {#if user.disabled}
  <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-red-50 dark:bg-red-900/40 text-red-600 dark:text-red-300 rounded-full">
  <UserX size={12} /> Disabled
  </span>
  {:else}
  <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-green-50 dark:bg-green-900/40 text-green-600 dark:text-green-300 rounded-full">
  ...
  <td class="px-4 py-3">
  {#if isOnline}
  <span class="inline-flex items-center gap-1 text-xs px-2 py-0.5 bg-green-50 dark:bg-green-900/40 text-green-700 dark:text-green-300 rounded-full tabular-nums">{user.active_sessions}</span>
 {:else}
 <span class="text-xs text-slate-400 dark:text-slate-500">0</span>
 {/if}
 </td>
 <td class="px-4 py-3 text-slate-500 dark:text-slate-400 text-xs">{new Date(user.created_at).toLocaleDateString()}</td>
 <td class="px-4 py-3 text-right">
 <div class="flex items-center justify-end gap-1">
 <button onclick={() => startEdit(user)} class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/30 rounded transition-colors" title="Edit user">
 <KeyRound size={14} />
 </button>
 {#if !isYou && isOnline}
 <button onclick={() => handleKick(user)} class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-orange-500 hover:bg-orange-50 dark:hover:bg-orange-900/30 rounded transition-colors" title="Kick user">
 <LogOut size={14} />
 </button>
 {/if}
 <button onclick={() => handleToggleDisabled(user)} class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded transition-colors"
 ...
 <!-- Pagination -->
 {#if total > 0}
 <div class="flex items-center justify-between px-4 py-3 border-t border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-800">
 ...
 class="p-1 rounded text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-warm-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Previous page">
 <ChevronLeft size={16} />
 </button>
 <span class="text-xs text-slate-600 dark:text-slate-300 px-2 tabular-nums">{currentPage} / {totalPages}</span>
 <button onclick={() => goToPage(currentPage + 1)} disabled={currentPage >= totalPages}
 class="p-1 rounded text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-200 dark:hover:bg-warm-700 disabled:opacity-30 disabled:cursor-not-allowed transition-colors" title="Next page">
 ...
 <div class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-4 mb-4">
 <h3 class="text-sm font-medium text-slate-700 dark:text-slate-200 mb-3">Create Permission</h3>
 <div class="grid grid-cols-2 gap-3 mb-3">
 <div>
 <label for="new-perm-name" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Name</label>
 <input id="new-perm-name" type="text" bind:value={newPermName}
 class="w-full px-3 py-1.5 border border-slate-300 dark:border-warm-700 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
 placeholder="e.g. Editor" />
 </div>
 <div>
 <label for="new-perm-key" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Slug <span class="text-slate-400 dark:text-slate-500">(auto-generated if empty)</span></label>
 ...
 <div class="mb-3">
 <label for="new-perm-desc" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Description</label>
 ...
 <div class="mb-3">
 <div class="text-xs text-slate-500 dark:text-slate-400 mb-2">Capabilities granted:</div>
 ...
 {selected ? 'bg-accent-50 border-accent-300 dark:bg-accent-900/30 dark:border-accent-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 hover:border-slate-300 dark:border-warm-700'}"
 >
 <div class="flex items-center justify-center w-3.5 h-3.5 mt-0.5 shrink-0 rounded border transition-colors
 {selected ? 'bg-accent-500 border-accent-500' : 'border-slate-300 dark:border-warm-700'}">
 {#if selected}<Check size={9} class="text-white" />{/if}
 </div>
 <div class="min-w-0">
 <code class="font-medium {selected ? 'text-accent-700 dark:text-accent-300' : 'text-slate-600 dark:text-slate-300'}">{known.key}</code>
 <div class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5 leading-snug">{known.description}</div>
 </div>
 </button>
 {/each}
 </div>
 ...
 <div class="mb-3 border-t border-slate-200 dark:border-warm-700 pt-3">
 <div class="flex items-baseline justify-between mb-1">
 <div class="text-xs font-medium text-slate-600 dark:text-slate-300">Path scoping <span class="text-slate-400 dark:text-slate-500 font-normal">(optional)</span></div>
 ...
 <div class="mb-2 p-2.5 bg-brand-50 border border-brand-200 dark:bg-brand-900/30 dark:border-brand-700 rounded text-[11px] text-slate-700 dark:text-slate-200 space-y-1.5 leading-relaxed">
 <div>
 ...
 <div class="border border-slate-200 dark:border-warm-700 rounded-md p-2 bg-slate-50 dark:bg-warm-800">
 <div class="flex items-center justify-between mb-1.5">
 <div class="flex items-center gap-2">
 <code class="text-[11px] font-medium text-slate-700 dark:text-slate-200">{k}</code>
 {#if !scopable}
 <span class="text-[9px] text-slate-400 dark:text-slate-500">(patterns ignored — not a path-bound capability)</span>
 {/if}
 </div>
 {#if scopable}
 <button type="button" onclick={() => addPattern('new', k)}
 class="text-[10px] text-brand-600 hover:text-accent-700 dark:text-accent-300 hover:underline">+ Add pattern</button>
 {/if}
 </div>
 {#if !scopable}
 <div class="text-[10px] text-slate-400 dark:text-slate-500 italic">applies globally</div>
 {:else if patterns.length === 0}
 <div class="text-[10px] text-slate-400 dark:text-slate-500 italic">all paths</div>
 {:else}
 <div class="space-y-1">
 ...
 class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded transition-colors" title="Remove pattern">
 <Trash2 size={12} />
 </button>
 </div>
 {/each}
 ...
 <thead>
 <tr class="bg-slate-50 dark:bg-warm-800 border-b border-slate-200 dark:border-warm-700">
 <th class="text-left px-4 py-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Name</th>
 <th class="text-left px-4 py-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Capabilities</th>
 <th class="text-right px-4 py-2.5 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wider">Actions</th>
 </tr>
 </thead>
 <tbody>
 {#each allPermissions as perm (perm.id)}
 <tr class="border-b border-slate-100 dark:border-warm-700 hover:bg-slate-50 dark:hover:bg-warm-700">
 <td class="px-4 py-3">
 <div class="font-medium text-slate-800 dark:text-slate-100">{perm.name}</div>
 {#if perm.description}
 <div class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{perm.description}</div>
 {/if}
 </td>
 <td class="px-4 py-3">
 ...
 <span class="inline-flex items-center gap-1">
  <code class="text-[10px] font-medium text-slate-600 dark:text-warm-100 bg-slate-100 dark:bg-warm-700 px-1.5 py-0.5 rounded">{k}</code>
  {#if pats.length > 0}
  ...
  <span class="text-[10px] font-medium text-amber-700 bg-amber-100 dark:bg-amber-900/40 dark:text-amber-200 px-1.5 py-0.5 rounded"
  title={pats.join('\n')}>
 ...
 {/if}
 <button onclick={() => startEditPerm(perm)} class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-brand-500 hover:bg-brand-50 dark:hover:bg-brand-900/30 rounded transition-colors" title="Edit permission">
 <KeyRound size={14} />
 </button>
 ...
 <button onclick={() => { confirmDeletePermId = null; }} class="px-2 py-1 text-xs bg-slate-200 dark:bg-warm-700 text-slate-600 dark:text-warm-200 rounded hover:bg-slate-300 dark:hover:bg-warm-600 transition-colors">Cancel</button>
 {:else}
 <button onclick={() => { confirmDeletePermId = perm.id; }} class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded transition-colors" title="Delete permission">
 <Trash2 size={14} />
 </button>
 {/if}
 </div>
 ...
 {:else}
 <tr>
 <td colspan="3" class="px-4 py-8 text-center text-slate-400 dark:text-slate-500 text-sm">
 ...
 <div class="bg-white dark:bg-warm-900 rounded-lg shadow-xl border border-slate-200 dark:border-warm-700 p-6 w-full max-w-lg">
 <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-4">
 Edit User: {editingUser.username}
 {#if editingUser.is_superadmin}
 <span class="ml-2 text-[10px] px-1.5 py-0.5 bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200 rounded font-medium">superadmin</span>
 {/if}
 </h3>

 <!-- Tabs inside modal -->
 <div class="flex gap-1 mb-4 border-b border-slate-200 dark:border-warm-700">
 <button onclick={() => { editTab = 'details'; }}
 class="px-3 py-1.5 text-xs font-medium border-b-2 transition-colors -mb-px {editTab === 'details' ? 'border-accent-500 text-accent-700 dark:text-accent-300' : 'border-transparent text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300'}">
 Details
 </button>
 <button onclick={() => { editTab = 'permissions'; }}
 class="px-3 py-1.5 text-xs font-medium border-b-2 transition-colors -mb-px {editTab === 'permissions' ? 'border-accent-500 text-accent-700 dark:text-accent-300' : 'border-transparent text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300'}">
 Permissions
 </button>
 </div>
 ...
 <div class="space-y-3">
 <div>
 <label for="edit-username" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Username</label>
 <input id="edit-username" type="text" bind:value={editUsername}
 class="w-full px-3 py-1.5 border border-slate-300 dark:border-warm-700 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent" />
 </div>
 <div>
 <label for="edit-password" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">New Password (leave empty to keep current)</label>
 ...
 <!-- Permissions tab -->
 {#if editingUser.is_superadmin}
 <div class="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 dark:bg-amber-900/30 dark:border-amber-700 rounded-lg text-xs text-amber-700 dark:text-amber-200">
  <ShieldCheck size={16} />
  <span>Superadmin users have all permissions automatically.</span>
 </div>
 {:else if loadingUserPerms}
 <div class="py-6 text-center text-sm text-slate-400 dark:text-slate-500">Loading permissions...</div>
 {:else if allPermissions.length === 0}
 <div class="py-6 text-center text-sm text-slate-400 dark:text-slate-500">No permissions defined. Create permissions in the Permissions tab first.</div>
 {:else}
 <div class="space-y-1 max-h-64 overflow-y-auto">
 {#each allPermissions as perm (perm.id)}
 {@const checked = editUserPermissionIds.includes(perm.id)}
 <label class="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-slate-50 dark:hover:bg-warm-700 cursor-pointer transition-colors">
 ...
 <div class="flex items-center gap-2">
 <Shield size={12} class="text-slate-400 dark:text-slate-500 shrink-0" />
 <code class="text-xs font-medium text-slate-700 dark:text-slate-200">{perm.key}</code>
 </div>
 <div class="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{perm.name}{perm.description ? ` — ${perm.description}` : ''}</div>
 </div>
 </label>
 {/each}
 </div>
 ...
 <div class="bg-white dark:bg-warm-900 rounded-lg shadow-xl border border-slate-200 dark:border-warm-700 p-6 w-full max-w-lg">
 <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100 mb-4">Edit Permission: {editingPerm.name}</h3>
 <div class="space-y-3">
 <div class="grid grid-cols-2 gap-3">
 <div>
 <label for="edit-perm-name" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Name</label>
 <input id="edit-perm-name" type="text" bind:value={editPermName}
 class="w-full px-3 py-1.5 border border-slate-300 dark:border-warm-700 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent" />
 </div>
 <div>
 <label for="edit-perm-key" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Slug</label>
 <input id="edit-perm-key" type="text" bind:value={editPermKey}
 class="w-full px-3 py-1.5 border border-slate-300 dark:border-warm-700 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent" />
 </div>
 </div>
 <div>
 <label for="edit-perm-desc" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Description</label>
 ...
 <div class="border-t border-slate-200 dark:border-warm-700 pt-3">
 <div class="flex items-baseline justify-between mb-1">
 <div class="text-xs font-medium text-slate-600 dark:text-slate-300">Path scoping <span class="text-slate-400 dark:text-slate-500 font-normal">(optional)</span></div>
 ...
 class="px-4 py-1.5 bg-white dark:bg-warm-900 border border-slate-300 dark:border-warm-700 text-slate-600 dark:text-slate-300 text-sm rounded-md hover:bg-slate-50 dark:hover:bg-warm-700 transition-colors">Cancel</button>
 <button onclick={handleSaveEditPerm}
 class="px-4 py-1.5 bg-accent-600 text-white text-sm rounded-md hover:bg-accent-700 transition-colors cursor-pointer">Save</button>
 </div>
 </div>
 </div>
 {/if}
