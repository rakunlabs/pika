...
<div>
 ...
 <div>
 <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Hooks</h2>
 <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Trigger HTTP webhooks or Kafka messages when files are created, updated, or deleted.</p>
 </div>
 <button
 ...
 <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">{editingHookIndex !== null ? 'Edit Hook' : 'Add Hook'}</h3>

 <!-- Hook Name -->
 <div class="mb-4">
 <label for="hook-name" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Hook Name</label>
 ...
 <!-- Event Types -->
 <div class="mb-4">
 <label for="hook-event-input" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Event Types</label>
 ...
 <button
 class="px-2 py-0.5 text-[10px] font-mono rounded border border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-900 text-slate-500 dark:text-slate-400 hover:bg-accent-50 dark:hover:bg-accent-900/30 hover:text-accent-700 hover:border-accent-200 transition-colors cursor-pointer"
 ...
 <!-- Filters -->
 <div class="mb-4 p-4 bg-slate-50 dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md">
 <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-3 uppercase tracking-wider">Filters (optional)</p>

 <!-- Mount filter -->
 <div class="mb-3">
 <label for="hook-mount-filter" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Mount Prefixes</label>
 ...
 class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10 bg-white dark:bg-warm-900" />
 <button class="px-3 py-2 text-sm text-white bg-accent-600 rounded-md hover:bg-accent-700 transition-colors cursor-pointer" onclick={addHookFilterMount}>Add</button>
 </div>
 <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500">Only fire for events on these mounts. Leave empty for all mounts.</p>
 {#if hookFilterMounts.length > 0}
 <div class="mt-2 flex flex-wrap gap-1.5">
 {#each hookFilterMounts as m, i}
 <span class="inline-flex items-center gap-1 px-2 py-1 bg-accent-50 border border-accent-200 rounded text-xs font-mono text-brand-700">
 {m}
 <button class="flex items-center justify-center w-3.5 h-3.5 p-0 border-none cursor-pointer bg-transparent text-brand-400 hover:text-red-500 transition-colors" onclick={() => removeHookFilterMount(i)} title="Remove">&times;</button>
 </span>
 {/each}
 </div>
 {/if}
 ...
 <!-- Path pattern -->
 <div>
 <label for="hook-path-pattern" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Path Pattern</label>
 <input id="hook-path-pattern" type="text" bind:value={hookFilterPathPattern} placeholder="e.g., *.pdf"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10 bg-white dark:bg-warm-900" />
 <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500">Glob pattern to match file paths. Leave empty for all paths.</p>
 </div>
 </div>
 ...
 <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Targets</p>
 <button
 class="flex items-center gap-1 px-2 py-1 text-xs text-accent-700 bg-accent-50 border border-accent-200 rounded hover:bg-accent-100 transition-colors cursor-pointer"
 ...
 <div class="flex items-center gap-3 p-3 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md">
 ...
 <span class="text-xs text-slate-600 dark:text-slate-300 font-mono truncate">{t.http.method || 'POST'} {t.http.url}</span>
 {:else if t.type === 'kafka' && t.kafka}
 <span class="text-xs text-slate-600 dark:text-slate-300 font-mono truncate">{t.kafka.topic} ({t.kafka.brokers.join(', ')})</span>
 {:else if t.type === 'redis' && t.redis}
 <span class="text-xs text-slate-600 dark:text-slate-300 font-mono truncate">{t.redis.channel} ({t.redis.addresses?.length ? t.redis.addresses.join(', ') : t.redis.address})</span>
 {:else if t.type === 'nats' && t.nats}
 <span class="text-xs text-slate-600 dark:text-slate-300 font-mono truncate">{t.nats.subject} ({t.nats.url})</span>
 {:else if t.type === 'log' && t.log}
 <span class="text-xs text-slate-600 dark:text-slate-300 font-mono truncate">{(t.log.level || 'info').toUpperCase()}{t.log.message ? `: ${t.log.message}` : ''}</span>
 {/if}
 </div>
 {#if t.body_template}
 <p class="mt-0.5 text-[10px] text-slate-400 dark:text-slate-500 truncate">Template: {t.body_template}</p>
 {/if}
 </div>
 <div class="flex items-center gap-1 shrink-0">
 <button class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-accent-600 hover:bg-accent-50 dark:hover:bg-accent-900/30 rounded transition-colors cursor-pointer" onclick={() => handleEditTarget(i)} title="Edit">
 <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/></svg>
 </button>
 <button class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded transition-colors cursor-pointer" onclick={() => removeTarget(i)} title="Remove">
 <Trash2 size={14} />
 </button>
 </div>
 </div>
 ...
 <p class="text-xs text-slate-400 dark:text-slate-500 mb-3">No targets added yet. Add at least one HTTP or Kafka target.</p>
 {/if}

 <!-- Add/Edit Target Form -->
 {#if showAddTarget}
 <div class="p-4 bg-slate-50 dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md mb-3">
 <h4 class="text-xs font-semibold text-slate-600 dark:text-slate-300 mb-3">{editingTargetIndex !== null ? 'Edit Target' : 'Add Target'}</h4>

 <!-- Target type -->
 <div class="mb-3">
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Type</label>
 <div class="flex gap-2">
 <button
 class="flex-1 px-3 py-2 text-xs font-medium rounded-md border transition-colors
 {targetType === 'http' ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 onclick={() => targetType = 'http'}
 >HTTP Webhook</button>
 <button
 class="flex-1 px-3 py-2 text-xs font-medium rounded-md border transition-colors
 {targetType === 'kafka' ? 'bg-purple-50 border-purple-300 text-purple-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 onclick={() => targetType = 'kafka'}
 >Kafka</button>
 <button
 class="flex-1 px-3 py-2 text-xs font-medium rounded-md border transition-colors
 {targetType === 'redis' ? 'bg-red-50 border-red-300 text-red-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 onclick={() => targetType = 'redis'}
 >Redis</button>
 <button
 class="flex-1 px-3 py-2 text-xs font-medium rounded-md border transition-colors
 {targetType === 'nats' ? 'bg-sky-50 border-sky-300 text-sky-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 onclick={() => targetType = 'nats'}
 >NATS</button>
 <button
 class="flex-1 px-3 py-2 text-xs font-medium rounded-md border transition-colors
 {targetType === 'log' ? 'bg-amber-50 border-amber-300 text-amber-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 ...
 <div>
 <label for="target-http-method" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Method</label>
 <select id="target-http-method" bind:value={targetHttpMethod}
 class="w-full px-2 py-2 text-sm border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 bg-white dark:bg-warm-900">
 ...
 <label for="target-http-url" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">URL</label>
 ...
 <div class="mb-3">
 <label for="target-http-timeout" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Timeout</label>
 <input id="target-http-timeout" type="text" bind:value={targetHttpTimeout} placeholder="30s"
 class="w-32 px-3 py-2 text-sm border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 ...
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs font-medium text-slate-500 dark:text-slate-400">Headers</label>
 <button class="text-[10px] text-accent-600 hover:text-accent-800 cursor-pointer" onclick={addTargetHeader}>+ Add Header</button>
 </div>
 {#each targetHttpHeaders as header, i}
 <div class="flex gap-2 mb-1.5">
 <input type="text" bind:value={header.key} placeholder="Header name"
 class="flex-1 px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 <input type="text" bind:value={header.value} placeholder="Value"
 class="flex-1 px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 <button class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 cursor-pointer" onclick={() => removeTargetHeader(i)} title="Remove">
 <Trash2 size={12} />
 </button>
 </div>
 {/each}
 ...
 <!-- Kafka target fields -->
 <div class="mb-3">
 <label for="target-kafka-broker" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Brokers</label>
 <div class="flex gap-2">
 ...
 class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 <button class="px-3 py-2 text-sm text-white bg-accent-600 rounded-md hover:bg-accent-700 transition-colors cursor-pointer" onclick={addTargetKafkaBroker}>Add</button>
 </div>
 ...
 <div class="mb-3">
 <label for="target-kafka-topic" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Topic</label>
 ...
 <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500 pl-5">Automatically create the topic on first produce. Requires broker <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">auto.create.topics.enable=true</code>.</p>
 </div>
 <div class="mb-3">
 <label for="target-kafka-key" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Key Template (optional)</label>
 <input id="target-kafka-key" type="text" bind:value={targetKafkaKeyTemplate} placeholder={'{{.Mount}}/{{.Path}}'}
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500">Go template for the Kafka message key. Default: mount/path</p>
 </div>

 <!-- Security -->
 <div class="p-3 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md mb-3">
 <p class="text-[11px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-3">Security</p>

 <!-- TLS -->
 <div class="mb-3">
 <label class="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-300 cursor-pointer mb-2">
 ...
 {targetKafkaTLSInputMode === 'path' ? 'bg-accent-50 border-accent-300 text-brand-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 onclick={() => targetKafkaTLSInputMode = 'path'}
 >File Path</button>
 <button
 class="px-2 py-1 text-[10px] font-medium rounded border transition-colors
 {targetKafkaTLSInputMode === 'pem' ? 'bg-accent-50 border-accent-300 text-brand-700' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 ...
 <div>
 <label for="kafka-tls-ca" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">CA File (optional)</label>
 <input id="kafka-tls-ca" type="text" bind:value={targetKafkaTLSCAFile} placeholder="/path/to/ca.pem"
 class="w-full px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 </div>
 <div>
 <label for="kafka-tls-cert" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Client Cert File (optional)</label>
 <input id="kafka-tls-cert" type="text" bind:value={targetKafkaTLSCertFile} placeholder="/path/to/cert.pem"
 class="w-full px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 </div>
 <div>
 <label for="kafka-tls-key" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Client Key File (optional)</label>
 ...
 <div>
 <label for="kafka-tls-cert-pem" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Client Certificate (optional)</label>
 ...
 <!-- SASL -->
 <div>
 <label for="kafka-sasl-type" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">SASL Authentication</label>
 <select id="kafka-sasl-type" bind:value={targetKafkaSASLType}
 class="w-full px-2 py-1.5 text-xs border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 bg-white dark:bg-warm-900 mb-2">
 ...
 <div>
 <label for="kafka-scram-algo" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Algorithm</label>
 <select id="kafka-scram-algo" bind:value={targetKafkaSASLAlgorithm}
 class="w-full px-2 py-1.5 text-xs border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 bg-white dark:bg-warm-900">
 ...
 {/if}
 <div>
 <label for="kafka-sasl-user" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Username</label>
 <input id="kafka-sasl-user" type="text" bind:value={targetKafkaSASLUser} placeholder="kafka-user"
 class="w-full px-2 py-1.5 text-xs border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 </div>
 <div>
 <label for="kafka-sasl-pass" class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1">Password</label>
 <div class="flex gap-1">
 <input id="kafka-sasl-pass" type={showKafkaSASLPass ? 'text' : 'password'} bind:value={targetKafkaSASLPass} placeholder="password"
 class="flex-1 px-2 py-1.5 text-xs border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 <button class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-warm-700 rounded-md cursor-pointer" onclick={() => showKafkaSASLPass = !showKafkaSASLPass} title={showKafkaSASLPass ? 'Hide' : 'Show'}>
 ...
 <div>
 <label for="target-redis-tls-ca" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">CA File (optional)</label>
 <input id="target-redis-tls-ca" type="text" bind:value={targetRedisTLSCAFile} placeholder="/path/to/ca.pem"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div>
 <label for="target-redis-tls-cert" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Cert File (optional)</label>
 <input id="target-redis-tls-cert" type="text" bind:value={targetRedisTLSCertFile} placeholder="/path/to/cert.pem"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div>
 <label for="target-redis-tls-key" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Key File (optional)</label>
 ...
 <div class="mb-3">
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Cluster Addresses</label>
 <div class="flex flex-wrap gap-1.5 mb-2">
 {#each targetRedisAddresses as addr, i}
 <span class="flex items-center gap-1 px-2 py-0.5 bg-slate-100 dark:bg-warm-900 text-slate-600 dark:text-slate-300 rounded text-xs font-mono">
 {addr}
 <button class="text-slate-400 dark:text-slate-500 hover:text-red-500 cursor-pointer" onclick={() => targetRedisAddresses = targetRedisAddresses.filter((_, j) => j !== i)}>&times;</button>
 </span>
 {/each}
 </div>
 <div class="flex gap-2">
 ...
 <button class="px-3 py-2 text-sm text-white bg-accent-600 rounded-md hover:bg-accent-700 transition-colors cursor-pointer"
 ...
 <div>
 <label for="target-redis-address" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Address</label>
 <input id="target-redis-address" type="text" bind:value={targetRedisAddress} placeholder="localhost:6379"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div>
 <label for="target-redis-db" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">DB (optional)</label>
 ...
 <div class="grid grid-cols-2 gap-3 mb-3">
 <div>
 <label for="target-redis-channel" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Channel</label>
 <input id="target-redis-channel" type="text" bind:value={targetRedisChannel} placeholder="pika-events"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div>
 <label for="target-redis-password" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Password (optional)</label>
 ...
 <div class="grid grid-cols-2 gap-3 mb-3">
 <div>
 <label for="target-nats-url" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">URL</label>
 <input id="target-nats-url" type="text" bind:value={targetNatsUrl} placeholder="nats://localhost:4222"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div>
 <label for="target-nats-subject" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Subject</label>
 <input id="target-nats-subject" type="text" bind:value={targetNatsSubject} placeholder="pika.events"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 </div>
 <div class="mb-3">
 <label for="target-nats-token" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Token (optional)</label>
 <input id="target-nats-token" type="password" bind:value={targetNatsToken} placeholder="Auth token"
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div class="grid grid-cols-2 gap-3 mb-3">
 <div>
 <label for="target-nats-username" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Username (optional)</label>
 <input id="target-nats-username" type="text" bind:value={targetNatsUsername} placeholder="Username"
 class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 <div>
 <label for="target-nats-password" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Password (optional)</label>
 ...
 <div class="grid grid-cols-4 gap-3 mb-3">
 <div>
 <label for="target-log-level" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Level</label>
 ...
 <div class="col-span-3">
 <label for="target-log-message" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Message Template (optional)</label>
 <input id="target-log-message" type="text" bind:value={targetLogMessage}
 placeholder={'file {{.Path}} {{.Type}} on {{.Mount}}'}
 class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10" />
 </div>
 </div>
 <p class="mb-3 text-[11px] text-slate-400 dark:text-slate-500">Default message (when empty) is the event type, e.g. <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">file.created</code>.</p>

 <!-- Log Fields -->
 <div class="mb-3">
 <div class="flex items-center justify-between mb-1">
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs font-medium text-slate-500 dark:text-slate-400">Fields</label>
 <button class="text-[10px] text-accent-600 hover:text-accent-800 cursor-pointer" onclick={addTargetLogField}>+ Add Field</button>
 </div>
 ...
 class="flex-1 px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500" />
 <button class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 cursor-pointer" onclick={() => removeTargetLogField(i)} title="Remove">
 <Trash2 size={12} />
 </button>
 </div>
 {/each}
 <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500">Each value is a Go template rendered against the Event. Rendered as slog attributes alongside <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">hook</code>.</p>
 </div>
 {/if}
 ...
 {#if targetType !== 'log'}
 <div class="mb-3">
 <label for="target-body-template" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Body Template (optional)</label>
 ...
 Go text/template. Available fields: <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.Type</code>, <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.Mount</code>, <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.Path</code>, <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.Size</code>, <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.Protocol</code>, <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.User</code>, <code class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]">.Timestamp</code>. Leave empty for default JSON payload.
 </p>
 </div>
 {/if}

 <div class="flex justify-end gap-2">
 <button class="px-3 py-1.5 text-xs text-slate-600 dark:text-slate-300 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md hover:bg-slate-50 dark:bg-warm-900 transition-colors cursor-pointer"
 onclick={() => { showAddTarget = false; resetTargetForm(); }}>Cancel</button>
 <button class="px-3 py-1.5 text-xs text-white bg-accent-600 rounded-md hover:bg-accent-700 transition-colors cursor-pointer"
 ...
 <p class="text-sm text-slate-500 dark:text-slate-400">No hooks configured</p>
 <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Add a hook to trigger notifications when files change</p>
 </div>
 {:else if !showAddHook}
 <div class="space-y-2">
 {#each hooks as h, i (h.name)}
 <div class="p-4 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-lg hover:border-slate-300 transition-colors">
 <div class="flex items-center gap-4">
 <div class="flex-1 min-w-0">
 <div class="flex items-center gap-2">
 <span class="text-sm font-medium text-slate-800 dark:text-slate-100">{h.name}</span>
 <span class="px-1.5 py-0.5 text-[10px] font-medium rounded {h.enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 dark:bg-warm-900 text-slate-500 dark:text-slate-400'}">
 {h.enabled ? 'Active' : 'Disabled'}
 </span>
 </div>
 <div class="mt-1 flex flex-wrap gap-1">
 {#each h.events as ev}
 <span class="px-1.5 py-0.5 text-[10px] font-mono rounded bg-accent-50 text-accent-700 border border-brand-100">{ev}</span>
 {/each}
 </div>
 {#if h.filter?.mounts?.length || h.filter?.path_pattern}
 <div class="mt-1 flex flex-wrap gap-1 items-center">
 <span class="text-[10px] text-slate-400 dark:text-slate-500">Filters:</span>
 {#if h.filter?.mounts}
 ...
 <div class="flex items-center gap-1 shrink-0">
 <button
 class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-emerald-500 hover:bg-emerald-50 rounded transition-colors cursor-pointer"
