<script lang="ts">
    ...
    let targetType = $state<"http" | "kafka" | "redis" | "nats" | "log">(
        "http",
    );
    ...
    let targetKafkaSASLAlgorithm = $state<"SCRAM-SHA-256" | "SCRAM-SHA-512">(
        "SCRAM-SHA-256",
    );
    ...
    function addTargetKafkaBroker() {
        ...
        if (!b) return;
        if (targetKafkaBrokers.includes(b)) {
    ...
    function handleAddTarget() {
        ...
        if (targetType === "http") {
            ...
            if (!targetRedisChannel.trim()) {
                addToast("Redis channel is required", "alert");
                return;
            }
            if (targetRedisCluster) {
                if (targetRedisAddresses.length === 0) {
                    addToast(
                        "At least one cluster address is required",
                        "alert",
                    );
                    return;
                }
            } else {
                if (!targetRedisAddress.trim()) {
                    addToast("Redis address is required", "alert");
                    return;
                }
            }
            const redisTls: import("@/lib/types/config").RedisTLS | undefined =
                targetRedisTLSEnabled
                    ? {
                          enabled: true,
            ...
            target = {
                type: "redis",
                redis: {
                    address: targetRedisCluster
                        ? undefined
                        : targetRedisAddress.trim(),
                    addresses: targetRedisCluster
                        ? [...targetRedisAddresses]
                        : undefined,
                    password: targetRedisPassword || undefined,
                    db: targetRedisCluster
                        ? undefined
                        : targetRedisDB || undefined,
            ...
            if (!targetNatsUrl.trim()) {
                addToast("NATS URL is required", "alert");
                return;
            }
            if (!targetNatsSubject.trim()) {
            ...
            const fieldsEntries = targetLogFields
                .filter((f) => f.key.trim())
    ...
    function handleEditTarget(index: number) {
        ...
        if (t.http) {
            ...
            targetHttpHeaders = t.http.headers
                ? Object.entries(t.http.headers).map(([key, value]) => ({
                      key,
                      value: String(value),
                  }))
                : [];
        ...
        if (t.kafka) {
            ...
            if (sec?.sasl?.length) {
                ...
                if (s.plain?.enabled) {
                    ...
                    targetKafkaSASLAlgorithm =
                        (s.scram.algorithm as
                            | "SCRAM-SHA-256"
        ...
        if (t.log) {
            targetLogLevel =
                (t.log.level as "debug" | "info" | "warn" | "error") || "info";
            targetLogMessage = t.log.message || "";
            targetLogFields = t.log.fields
                ? Object.entries(t.log.fields).map(([key, value]) => ({
                      key,
                      value: String(value),
                  }))
                : [];
...
<div>
                ...
                {#if hookTargets.length > 0}
                    <div class="space-y-2 mb-3">
                        {#each hookTargets as t, i}
                            <div
                                class="flex items-center gap-3 p-3 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md"
                            >
 ...
 {targetType === 'http'
                                        ...
                                        : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
 ...
 {targetType === 'log'
                        ...
                        {#if targetType === "http"}
                            ...
                            <div class="mb-3">
                                ...
                                >
                                    ...
                                    >
                                    <button
                                        class="text-[10px] text-accent-600 hover:text-accent-800 cursor-pointer"
                                        onclick={addTargetHeader}
                                        >+ Add Header</button
                                    >
                                </div>
                                {#each targetHttpHeaders as header, i}
                                    <div class="flex gap-2 mb-1.5">
                                        ...
                                        />
                                        <button
                                            class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 cursor-pointer"
                                            onclick={() =>
                                                removeTargetHeader(i)}
                                            title="Remove"
                                        >
                                            <Trash2 size={12} />
                                        </button>
                        ...
                        {:else if targetType === "kafka"}
                            ...
                            <div class="mb-3">
                                ...
                                <div class="flex gap-2">
                                    <input
                                        ...
                                        type="text"
                                        bind:value={targetKafkaBrokerInput}
                                        placeholder="kafka:9092"
                                        onkeydown={(e) => {
                                            if (e.key === "Enter") {
                                                e.preventDefault();
                                                addTargetKafkaBroker();
                                            }
                                        }}
                                        class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10"
                                    />
                                ...
                                {#if targetKafkaBrokers.length > 0}
                                    <div class="mt-1.5 flex flex-wrap gap-1.5">
                                        {#each targetKafkaBrokers as b, i}
                                            <span
                                                class="inline-flex items-center gap-1 px-2 py-1 bg-purple-50 border border-purple-200 rounded text-xs font-mono text-purple-700"
                                            >
                                                {b}
                                                <button
                                                    class="flex items-center justify-center w-3.5 h-3.5 p-0 border-none cursor-pointer bg-transparent text-purple-400 hover:text-red-500 transition-colors"
                                                    onclick={() =>
                                                        removeTargetKafkaBroker(
                                                            i,
                                                        )}
                                                    title="Remove"
                                                    >&times;</button
                            ...
                            <div class="mb-3">
                                ...
                                <p
                                    class="mt-1 text-[11px] text-slate-400 dark:text-slate-500 pl-5"
                                >
                                    Automatically create the topic on first
                                    produce. Requires broker <code
                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                        >auto.create.topics.enable=true</code
                                    >.
                                </p>
                            </div>
                            <div class="mb-3">
                                <label
                                    for="target-kafka-key"
                                    class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1"
                                    >Key Template (optional)</label
                                >
                                <input
                                    id="target-kafka-key"
                                    type="text"
                                    bind:value={targetKafkaKeyTemplate}
                                ...
                                <p
                                    class="mt-1 text-[11px] text-slate-400 dark:text-slate-500"
                                >
                                    Go template for the Kafka message key.
                                    Default: mount/path
                                </p>
                            </div>
                            ...
                            <!-- Security -->
                            <div
                                class="p-3 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md mb-3"
                            >
                                <p
                                ...
                                <div class="mb-3">
                                    ...
                                    >
                                        <input
                                            type="checkbox"
                                            bind:checked={targetKafkaTLSEnabled}
                                            class="rounded border-slate-300"
                                        />
                                        Enable TLS
                                    </label>
 ...
 {targetKafkaTLSInputMode === 'path'
                                                    ? 'bg-accent-50 border-accent-300 text-brand-700'
                                                    : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:bg-warm-900'} cursor-pointer"
                                                onclick={() =>
                                                    (targetKafkaTLSInputMode =
                                                        "path")}
 ...
 {targetKafkaTLSInputMode === 'pem'
                                        ...
                                        {#if targetKafkaTLSInputMode === "path"}
                                            <div
                                                class="grid grid-cols-1 gap-2 pl-5"
                                            >
                                                <div>
                                                    <label
                                                        for="kafka-tls-ca"
                                                        class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1"
                                                        >CA File (optional)</label
                                                    >
                                                    <input
                                                        id="kafka-tls-ca"
                                                        type="text"
                                            ...
                                            >
                                                <div>
                                                    ...
                                                    <textarea
                                                        ...
                                                        class="w-full px-2 py-1.5 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 resize-y"
                                                    ></textarea>
                                                </div>
                                                <div>
                                                    <label
                                                        for="kafka-tls-cert-pem"
                                                        class="block text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-1"
                                                        >Client Certificate
                                                        (optional)</label
                                                    >
                                                ...
                                                >
                                                    Supports: inline PEM text, <code
                                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded"
                                                        >raw://mount/path</code
                                                    >
                                                    (from raw mounts), or
                                                    <code
                                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded"
                                                        >config://file/key</code
                                                    >
                                                    (from config store).
                                                    Structured JSON/YAML/TOML
                                                    files can use JSON Pointer
                                                    selectors like
                                                    <code
                                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded"
                                                        >#/client/cert</code
                                ...
                                <div>
                                    ...
                                    <select
                                        id="kafka-sasl-type"
                                        bind:value={targetKafkaSASLType}
                                        ...
                                        <div class="pl-5 space-y-2">
                                            {#if targetKafkaSASLType === "scram"}
                                                <div>
                                                    ...
                                                    >
                                                        <option
                                                            value="SCRAM-SHA-256"
                                                            >SCRAM-SHA-256</option
                                                        >
                                                        <option
                                                            value="SCRAM-SHA-512"
                                                            >SCRAM-SHA-512</option
                                                        >
                                                    </select>
                                                </div>
                                            {/if}
                                            ...
                                            <div>
                                                ...
                                                <div class="flex gap-1">
                                                    ...
                                                    />
                                                    <button
                                                        class="p-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-warm-700 rounded-md cursor-pointer"
                                                        onclick={() =>
                                                            (showKafkaSASLPass =
                                                                !showKafkaSASLPass)}
                                                        title={showKafkaSASLPass
                                                            ? "Hide"
                                                            : "Show"}
                                                    >
                                                        {#if showKafkaSASLPass}<EyeOff
                                                                size={12}
                                                            />{:else}<Eye
                                                                size={12}
                                                            />{/if}
                        ...
                        {:else if targetType === "redis"}
                            ...
                            <div class="flex gap-4 mb-3">
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        bind:checked={targetRedisCluster}
                                        class="rounded border-slate-300"
                                    />
                                    Cluster mode
                                </label>
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        bind:checked={targetRedisTLSEnabled}
                                        class="rounded border-slate-300"
                                    />
                                    Use TLS
                                </label>
                            ...
                            {#if targetRedisTLSEnabled}
                                <div class="grid grid-cols-3 gap-3 mb-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            type="text"
                                            bind:value={targetRedisTLSCAFile}
                                            placeholder="/path/to/ca.pem"
                                            class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            type="text"
                                            bind:value={targetRedisTLSCertFile}
                                            placeholder="/path/to/cert.pem"
                                            class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10"
                                        />
                                    </div>
                                    <div>
                                        <label
                                            for="target-redis-tls-key"
                                            class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1"
                                            >Key File (optional)</label
                                        >
                                        <input
                                            id="target-redis-tls-key"
                                            type="text"
                                            bind:value={targetRedisTLSKeyFile}
                                            placeholder="/path/to/key.pem"
                                            class="w-full px-3 py-2 text-sm font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10"
                                        />
                                    </div>
                                ...
                                <p
                                    class="mb-3 text-[11px] text-slate-400 dark:text-slate-500"
                                >
                                    Paths support references: <code
                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                        >raw://mount/path</code
                                    >
                                    (from raw mounts) or
                                    <code
                                        ...
                                        >config://key</code
                                    >
                                    (from config store), plus JSON Pointer
                                    selectors like
                                    <code
                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                        >#/key</code
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="flex flex-wrap gap-1.5 mb-2">
                                        {#each targetRedisAddresses as addr, i}
                                            <span
                                                class="flex items-center gap-1 px-2 py-0.5 bg-slate-100 dark:bg-warm-900 text-slate-600 dark:text-slate-300 rounded text-xs font-mono"
                                            >
                                                {addr}
                                                <button
                                                    class="text-slate-400 dark:text-slate-500 hover:text-red-500 cursor-pointer"
                                                    onclick={() =>
                                                        (targetRedisAddresses =
                                                            targetRedisAddresses.filter(
                                                                (_, j) =>
                                                                    j !== i,
                                                            ))}>&times;</button
                                                >
                                    ...
                                    <div class="flex gap-2">
                                        <input
                                            type="text"
                                            bind:value={targetRedisAddressInput}
                            ...
                            <div class="grid grid-cols-2 gap-3 mb-3">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        type="password"
                                        bind:value={targetRedisPassword}
                                        placeholder="Password"
                                        class="w-full px-3 py-2 text-sm border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10"
                                    />
                                </div>
                        ...
                        {:else if targetType === "log"}
                            ...
                            <div class="grid grid-cols-4 gap-3 mb-3">
                                <div>
                                    <label
                                        for="target-log-level"
                                        class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1"
                                        >Level</label
                                    >
                                    <select
                                        id="target-log-level"
                                        bind:value={targetLogLevel}
                                        class="w-full px-2 py-2 text-sm border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 bg-white dark:bg-warm-900"
                                    >
                            ...
                            <p
                                class="mb-3 text-[11px] text-slate-400 dark:text-slate-500"
                            >
                                Default message (when empty) is the event type,
                                e.g. <code
                                    class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                    >file.created</code
                                >.
                            ...
                            <div class="mb-3">
                                ...
                                <p
                                    class="mt-1 text-[11px] text-slate-400 dark:text-slate-500"
                                >
                                    Each value is a Go template rendered against
                                    the Event. Rendered as slog attributes
                                    alongside <code
                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                        >hook</code
                                    >.
 ...
 which renders its own Message and Fields directly from the Event. -->
                        {#if targetType !== "log"}
                            <div class="mb-3">
                                <label
                                    for="target-body-template"
                                    class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1"
                                    >Body Template (optional)</label
                                >
                                <textarea
                                    id="target-body-template"
                                    bind:value={targetBodyTemplate}
                                    rows={3}
                                    placeholder={'{"text": "File {{.Path}} was {{.Type}} on {{.Mount}}"}'}
                                    class="w-full px-3 py-2 text-xs font-mono border border-slate-200 dark:border-warm-700 rounded-md focus:outline-none focus:border-accent-500 focus:ring-2 focus:ring-accent-500/10 resize-y"
                                ></textarea>
                                <p
                                    class="mt-1 text-[11px] text-slate-400 dark:text-slate-500"
                                >
                                    Go text/template. Available fields: <code
                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                        >.Type</code
                                    >,
                                    <code
                                    ...
                                    <code
                                        class="px-0.5 bg-slate-100 dark:bg-warm-900 rounded text-[10px]"
                                        >.Timestamp</code
                                    >. Leave empty for default JSON payload.
                                </p>
                            </div>
                        {/if}
                        ...
                        <div class="flex justify-end gap-2">
                            <button
                                class="px-3 py-1.5 text-xs text-slate-600 dark:text-slate-300 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md hover:bg-slate-50 dark:bg-warm-900 transition-colors cursor-pointer"
                                onclick={() => {
                                    showAddTarget = false;
                                    resetTargetForm();
                                }}>Cancel</button
    ...
    {:else if !showAddHook}
        <div class="space-y-2">
            {#each hooks as h, i (h.name)}
                <div
                    class="p-4 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-lg hover:border-slate-300 transition-colors"
                >
                    <div class="flex items-center gap-4">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="mt-1 flex flex-wrap gap-1">
                                {#each h.events as ev}
                                    <span
                                        class="px-1.5 py-0.5 text-[10px] font-mono rounded bg-accent-50 text-accent-700 border border-brand-100"
                                        >{ev}</span
                                    >
                                {/each}
                            </div>
                                ...
                                >
                                    ...
                                    {#if h.filter?.mounts}
                                        {#each h.filter.mounts as m}
                                            <span
                                                class="px-1.5 py-0.5 text-[10px] font-mono rounded bg-amber-50 text-amber-600 border border-amber-100"
                                                >{m}</span
                                            >
                                        {/each}
                                    {/if}
                                    {#if h.filter?.path_pattern}
                                        <span
                                            class="px-1.5 py-0.5 text-[10px] font-mono rounded bg-violet-50 text-violet-600 border border-violet-100"
