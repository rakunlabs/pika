<script lang="ts">
     ...
     const PIKABW_MAGIC = new Uint8Array([
          0x50, 0x49, 0x4b, 0x41, 0x42, 0x57, 0x00, 0x01,
     ]);
     ...
     async function doExportBackup() {
          ...
          try {
               ...
               const summary = encryptionPassword.trim()
     ...
     async function handleFileSelect(event: Event) {
          ...
          if (input.files && input.files.length > 0) {
               ...
               try {
                    const headerBuf = await input.files[0]
                         .slice(0, HEADER_SIZE)
                         .arrayBuffer();
     ...
     function handleImportBackup() {
          ...
          if (importFileIsEncrypted && !importEncryptionPassword.trim()) {
               addToast(
                    "Encryption password is required for encrypted backups",
                    "alert",
               );
               return;
          }
     ...
     async function doImportBackup() {
          ...
          try {
               ...
               if (importEncryptionPassword.trim()) {
                    headers["X-Encryption-Password"] =
                         importEncryptionPassword.trim();
               }
               if (wipeBeforeRestore) {
               ...
               addToast(
                    wipeBeforeRestore
                         ? "Database wiped and backup restored"
...
<div>
     <div class="mb-4">
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
               Export the entire database as a <code
                    class="text-[11px] bg-slate-100 dark:bg-warm-900 px-1 py-0.5 rounded"
                    >.pikabw</code
               > stream, or restore from a previous one. Backups capture every key
               (configs, users, tokens, settings).
          </p>
     </div>
     ...
     >
          ...
          >
               ...
               >
                    <RefreshCw
                         size={12}
                         class={isLoadingVersion ? "animate-spin" : ""}
                    />
                    Refresh
               </button>
          </div>
     ...
     >
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">
               Pika streams a binary <code
                    class="text-[11px] bg-slate-100 dark:bg-warm-900 px-1 py-0.5 rounded"
                    >.pikabw</code
               > file. Plain by default; encrypted with XChaCha20-Poly1305 when an
               encryption password is supplied.
          </p>
          ...
          <div class="mb-4">
               ...
               <div class="space-y-2">
                    <label
                         class="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-300 cursor-pointer"
                    >
                         <input
                              type="radio"
                         ...
                         <span>
                              ...
                              >
                              <span
                                   class="block text-[11px] text-slate-400 dark:text-slate-500"
                                   >Snapshot of the entire current database.</span
                              >
                         </span>
                    </label>
                    <label
                    ...
                    >
                         ...
                         <span>
                              <span
                                   class="font-medium text-slate-700 dark:text-slate-200"
                                   >Incremental (since)</span
                              >
                              <span
                                   class="block text-[11px] text-slate-400 dark:text-slate-500"
                                   >Only entries newer than the supplied
                                   version. Useful for chained incremental
                                   backups.</span
                              >
                         </span>
                    </label>
                    ...
                    >
                         ...
                         <span>
                              <span
                                   class="font-medium text-slate-700 dark:text-slate-200"
                                   >Point-in-time (until)</span
                              >
                              <span
                                   class="block text-[11px] text-slate-400 dark:text-slate-500"
                                   >Snapshot containing only entries with
                                   version ≤ the supplied value.</span
                              >
                         </span>
                    </label>
               </div>
               ...
               {#if backupMode === "since"}
                    <div class="mt-3">
                         ...
                         >
                              Entries newer than this version will be included.
                              Use the value from a previous full backup to
                              chain.
                         </p>
                    </div>
               {:else if backupMode === "until"}
                    <div class="mt-3">
                         ...
                         <p
                              class="mt-1 text-[11px] text-slate-400 dark:text-slate-500"
                         >
                              Restoring this archive returns the database to its
          ...
          <div class="mb-4">
               <label
                    for="export-encryption-password"
                    class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5"
               ...
               <div class="relative">
                    ...
                    >
                         {#if showEncryptionPassword}<EyeOff
                                   size={15}
                              />{:else}<Eye size={15} />{/if}
                    </button>
               </div>
               <p class="mt-1 text-[11px] text-slate-400 dark:text-slate-500">
                    If set, the backup payload is encrypted (ChaCha20-Poly1305).
                    The same password is required at restore time.
               </p>
          </div>
               ...
               >
                    <div class="flex items-start gap-3 mb-4">
                         ...
                         <div>
                              ...
                              >
                                   The backup will contain every config, user,
                                   token, and setting in plain bytes. Anyone who
                                   obtains the file can read its contents and
                                   forge a working pika instance.
                              </p>
                              <p
                                   class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mt-2"
                              >
                                   To encrypt the backup, cancel and enter an
                                   encryption password above.
                              </p>
                         </div>
                    </div>
     ...
     >
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 mb-4">
               Upload a previously exported <code
                    class="text-[11px] bg-slate-100 dark:bg-warm-900 px-1 py-0.5 rounded"
                    >.pikabw</code
               > file. Restore is an upsert — keys present in the backup overwrite
               any matching keys in the running DB. Keys absent from the backup are
               preserved.
          </p>
               ...
               <div class="flex items-center gap-2">
                    ...
                    >
                         ...
                         <span
                              ...
                              >{importFileName ||
                                   "Choose a .pikabw backup file..."}</span
                         >
                         <input
                              id="backup-file"
                              type="file"
               ...
               >
                    ...
                    <div class="relative">
                         <input
                              ...
                              type={showImportEncryptionPassword
                                   ? "text"
                                   : "password"}
                         ...
                         <button
                              ...
                              class="absolute right-2 top-1/2 -translate-y-1/2 p-0.5 text-slate-400 dark:text-slate-500 bg-transparent border-none cursor-pointer hover:text-slate-600 dark:text-slate-300 transition-colors"
                              onclick={() =>
                                   (showImportEncryptionPassword =
                                        !showImportEncryptionPassword)}
                              title={showImportEncryptionPassword
                                   ? "Hide"
                                   : "Show"}
                         >
                              {#if showImportEncryptionPassword}<EyeOff
                                        size={15}
                                   />{:else}<Eye size={15} />{/if}
                         </button>
                    </div>
 ...
 behind an explicit checkbox + confirm modal. -->
               ...
               >
                    ...
                    <span>
                         <span class="font-medium"
                              >Wipe existing data first</span
                         >
                         <span
                              class="block text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 leading-relaxed"
                         >
                              Drops every key in the running database before
                              applying the backup. The result matches the backup
                              exactly. Use this to roll back to a snapshot —
                              otherwise leave unchecked for an additive restore.
                         </span>
                    </span>
               </label>
          </div>
  ...
  {wipeBeforeRestore
               ...
               disabled={isImporting ||
                    !importFile ||
                    importFileDBVersion === null ||
                    (importFileIsEncrypted && !importEncryptionPassword.trim())}
          >
               <Upload size={14} />
               {isImporting
                    ? "Restoring..."
                    ...
                    <div class="flex items-start gap-3 mb-4">
                         ...
                         <div>
                              ...
                              <p
                                   class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed"
                              >
                                   Every key in the running database will be
                                   dropped before the backup is applied. This
                                   includes data the backup does not contain —
                                   configs, users, tokens, and settings that
                                   were added after the backup was taken.
                              </p>
                              <p
                                   class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mt-2"
                              >
                                   The backup file is validated before any wipe
                                   runs, so a bad file aborts the operation
                                   cleanly. Once the wipe starts, however, it is
                                   irreversible.
                              </p>
                              {#if importFileDBVersion !== null && currentDBVersion !== null && importFileDBVersion < currentDBVersion}
                                   ...
                                   >
                                        ...
                                        <span class="font-mono"
                                             >{importFileDBVersion}</span
                                        >, which is older than the current
                                        <span class="font-mono"
                                             >{currentDBVersion}</span
                                        >. You're rolling back to an earlier
                                        snapshot.
                                   </p>
                              {/if}
                         </div>
