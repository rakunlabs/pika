<script lang="ts">
 ...
 if (!confirm('Reset all appearance preferences to defaults? This will be saved immediately.')) return;
 try {
...
<div>
 ...
 Personal display preferences stored against your user — they follow you across devices.
 </p>
 </div>
 ...
 <!-- Editor preferences -->
 <section class="p-5 bg-white dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-lg shadow-sm mb-4">
 <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1">Editor</h3>
 <p class="text-xs text-slate-500 dark:text-slate-400 mb-3">
 Theme, font and behavior of the code editor. The preview below updates as you change settings.
 </p>
 ...
 onchange={(e) => setEditorTheme((e.currentTarget as HTMLSelectElement).value as EditorThemeKey)}
 >
 ...
 <!-- Font family -->
 <label class="flex flex-col gap-1">
 <span class="text-xs font-medium text-slate-600 dark:text-slate-300">Font family</span>
 <select
 class="px-2 py-1.5 text-sm border border-slate-200 dark:border-warm-700 bg-white dark:bg-warm-900 text-slate-800 dark:text-slate-100 rounded-md cursor-pointer"
 value={prefsStore.editor.font_family}
 onchange={(e) => setEditorFontFamily((e.currentTarget as HTMLSelectElement).value)}
 >
 ...
 Font size: <span class="font-mono text-slate-800 dark:text-slate-100">{prefsStore.editor.font_size}px</span>
 </span>
 <input
 type="range"
 ...
 oninput={(e) => setEditorFontSize(parseInt((e.currentTarget as HTMLInputElement).value, 10))}
 ...
 onchange={(e) => setEditorLineWrap((e.currentTarget as HTMLInputElement).checked)}
 class="cursor-pointer"
 />
 <span class="text-sm text-slate-700 dark:text-slate-200">Wrap long lines</span>
 </label>
 </div>
    ...
    <div class="mt-4">
      <span class="block text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">Preview</span>
      <div
        class="border border-slate-200 dark:border-warm-700 rounded-md overflow-auto bg-white dark:bg-warm-900"
        style="height: {Math.min(420, Math.max(176, prefsStore.editor.font_size * 9))}px"
      >
 ...
 <div class="mt-4 px-3 py-2 text-xs text-amber-800 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-md">
 You have unsaved appearance changes. Click <strong>Save</strong> to persist them.
 </div>
 {/if}
</div>
