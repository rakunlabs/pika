...
<div>
 ...
 <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Provision and refresh users from external directories. Each source owns its synced users + group-mapped permissions; admin grants are preserved.</p>
 </div>
 {#if editingIndex === null}
 <button onclick={startNew}
 class="flex items-center gap-1.5 px-3 py-1.5 bg-accent-600 text-white text-xs font-medium rounded-md hover:bg-accent-700 transition-colors cursor-pointer">
 ...
 <!-- ── List view ── -->
 {#if sources.length === 0}
 <div class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-8 text-center text-sm text-slate-500 dark:text-slate-400">
 ...
 <div class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-3">
 <div class="flex items-start justify-between gap-3">
 <div class="min-w-0">
 <div class="flex items-center gap-2 flex-wrap">
 <span class="font-medium text-slate-800 dark:text-slate-100 text-sm">{src.name}</span>
 <code class="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-warm-900 px-1.5 py-0.5 rounded">{src.id}</code>
 <span class="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded font-medium
 {src.enabled ? 'bg-green-50 text-green-700' : 'bg-slate-100 dark:bg-warm-900 text-slate-500 dark:text-slate-400'}">
 {src.enabled ? 'enabled' : 'disabled'}
 </span>
 <span class="text-[10px] text-slate-500 dark:text-slate-400">{status?.schedule_human ?? 'manual'}</span>
 </div>
 <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 truncate">
 {src.ldap?.address ?? ''} · {src.ldap?.user_base_dn ?? ''}
 </div>
 {#if status?.last}
 <div class="text-[11px] text-slate-500 dark:text-slate-400 mt-1.5">
 Last run: {formatDate(status.last.finished_at)} ·
 Found <span class="text-slate-700 dark:text-slate-200 font-medium">{status.last.found}</span> ·
 Created <span class="text-green-700 font-medium">{status.last.created}</span> ·
 Updated <span class="text-brand-700 font-medium">{status.last.updated}</span> ·
 ...
 class="px-2 py-1 text-[11px] bg-accent-50 text-brand-700 rounded hover:bg-accent-100 disabled:opacity-50 transition-colors flex items-center gap-1">
 <RefreshCw size={11} class={runningSourceId === src.id ? 'animate-spin' : ''} />
 {runningSourceId === src.id ? 'Running…' : 'Sync now'}
 </button>
 <button onclick={() => startEdit(idx)}
 class="px-2 py-1 text-[11px] bg-slate-100 dark:bg-warm-900 text-slate-700 dark:text-slate-200 rounded hover:bg-slate-200 dark:hover:bg-warm-700 transition-colors">Edit</button>
 ...
 <button onclick={() => { confirmDeleteIdx = null; }} class="px-2 py-1 text-[11px] bg-slate-200 dark:bg-warm-900 text-slate-600 dark:text-slate-300 rounded hover:bg-slate-300 dark:hover:bg-warm-600 transition-colors">Cancel</button>
 {:else}
 <button onclick={() => { confirmDeleteIdx = idx; }} class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded transition-colors" title="Delete source">
 <Trash2 size={14} />
 </button>
 {/if}
 </div>
 ...
 {:else}
 <!-- ── Editor view ── -->
 <div class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-4 space-y-4">
 <h3 class="text-sm font-semibold text-slate-800 dark:text-slate-100">{editingIndex === -1 ? 'New sync source' : `Edit "${sources[editingIndex]?.name}"`}</h3>

 <!-- Identity -->
 <div class="grid grid-cols-2 gap-3">
 <div>
 <label for="src-id" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Source ID</label>
 <input id="src-id" type="text" bind:value={formId} disabled={editingIndex !== -1}
 placeholder="ldap-prod"
 class="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent-500 disabled:bg-slate-50 dark:bg-warm-900 disabled:text-slate-500 dark:text-slate-400" />
 <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">Stable handle. Used internally as the user_identities provider; cannot be changed after save.</p>
 </div>
 <div>
 <label for="src-name" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Display name</label>
 <input id="src-name" type="text" bind:value={formName}
 placeholder="Production LDAP"
 class="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-accent-500" />
 </div>
 </div>

 <label class="flex items-center gap-2 text-sm">
 <input type="checkbox" bind:checked={formEnabled} class="rounded border-slate-300 text-accent-600 focus:ring-accent-500" />
 Enabled (periodic schedule fires only when checked)
 </label>

 <!-- Connection -->
 <fieldset class="border border-slate-200 dark:border-warm-700 rounded-md p-3 space-y-3">
 <legend class="text-xs font-medium text-slate-600 dark:text-slate-300 px-1">LDAP connection</legend>
 <div class="grid grid-cols-2 gap-3">
 <div class="col-span-2">
 <label for="ldap-addr" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Address</label>
 <input id="ldap-addr" type="text" bind:value={formAddress}
 placeholder="ldap://ad.example.com:389 or ldaps://ad.example.com:636"
 class="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent-500" />
 <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">Bare host:port works too — TLS toggle below picks the scheme.</p>
 </div>
 </div>
 <div class="flex gap-4">
 <label class="flex items-center gap-2 text-sm">
 <input type="checkbox" bind:checked={formTLS} class="rounded border-slate-300 text-accent-600 focus:ring-accent-500" />
 TLS
 </label>
 <label class="flex items-center gap-2 text-sm">
 <input type="checkbox" bind:checked={formInsecureSkip} class="rounded border-slate-300 text-accent-600 focus:ring-accent-500" />
 ...
 <div class="grid grid-cols-2 gap-3">
 <div>
 <label for="ldap-binddn" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Bind DN</label>
 <input id="ldap-binddn" type="text" bind:value={formBindDN}
 placeholder="cn=admin,dc=example,dc=com"
 class="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent-500" />
 </div>
 <div>
 <label for="ldap-bindpw" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Bind password</label>
 <div class="relative">
 ...
 class="w-full px-3 py-1.5 pr-9 border border-slate-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent-500" />
 <button type="button" onclick={() => { showBindPassword = !showBindPassword; }}
 class="absolute right-2 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:text-slate-300">
 ...
 <!-- Search -->
 <fieldset class="border border-slate-200 dark:border-warm-700 rounded-md p-3 space-y-3">
 <legend class="text-xs font-medium text-slate-600 dark:text-slate-300 px-1">Search</legend>
 <div>
 <label for="ldap-base" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">User Base DN</label>
 ...
 <div class="col-span-2">
 <label for="ldap-filter" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">User filter</label>
 <input id="ldap-filter" type="text" bind:value={formUserFilter}
 placeholder="(objectClass=person)"
 class="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent-500" />
 </div>
 <div>
 <label for="ldap-page" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Page size</label>
 ...
 <!-- Attribute mapping -->
 <fieldset class="border border-slate-200 dark:border-warm-700 rounded-md p-3 space-y-2">
 <legend class="text-xs font-medium text-slate-600 dark:text-slate-300 px-1">Attribute mapping</legend>
 <p class="text-[11px] text-slate-500 dark:text-slate-400 -mt-1">Each field is the LDAP attribute name to read for that pika user field. Leave blank to skip.</p>
 ...
 <div>
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Subject</label>
 ...
 <div>
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Display name</label>
 <input type="text" bind:value={attrDisplayName} placeholder="e.g. displayName, gecos, cn"
 class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500" />
 </div>
 <div>
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Given name</label>
 <input type="text" bind:value={attrGivenName} placeholder="optional · e.g. givenName"
 class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500" />
 </div>
 <div>
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Surname</label>
 <input type="text" bind:value={attrSurname} placeholder="optional · e.g. sn"
 class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500" />
 </div>
 <div class="col-span-2">
 <!-- svelte-ignore a11y_label_has_associated_control -->
 <label class="block text-xs text-slate-500 dark:text-slate-400 mb-1">Groups</label>
 ...
 <fieldset class="border border-slate-200 dark:border-warm-700 rounded-md p-3 space-y-2">
 <legend class="text-xs font-medium text-slate-600 dark:text-slate-300 px-1">Group → permissions</legend>
 <p class="text-[11px] text-slate-500 dark:text-slate-400 -mt-1">When a user's Groups attribute (e.g. memberOf) contains a value listed here, they're granted the selected permissions. Match is verbatim — for AD-style memberOf, paste the full group DN.</p>
 {#if permissions.length === 0}
 <div class="text-[11px] text-slate-400 dark:text-slate-500 italic py-2">No permissions defined yet. Create them under Users → Permissions first.</div>
 {/if}
 {#each groupRows as row, idx (idx)}
 <div class="border border-slate-200 dark:border-warm-700 rounded p-2 space-y-1.5 bg-slate-50 dark:bg-warm-900/50">
 ...
 class="flex-1 px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500" />
 <button type="button" onclick={() => removeGroupRow(idx)}
 class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded transition-colors" title="Remove">
 <Trash2 size={12} />
 </button>
 </div>
 {#if permissions.length > 0}
 ...
 {checked ? 'bg-accent-50 border-accent-300 text-brand-700 font-medium' : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-600 dark:text-slate-300 hover:border-slate-300'}">
 {perm.name}
 </button>
 {/each}
 </div>
 ...
 class="text-[11px] text-accent-700 hover:underline cursor-pointer">+ Add group mapping</button>
 </fieldset>

 <!-- Schedule -->
 <fieldset class="border border-slate-200 dark:border-warm-700 rounded-md p-3 space-y-2">
 <legend class="text-xs font-medium text-slate-600 dark:text-slate-300 px-1">Schedule</legend>
 ...
 class="w-20 px-2 py-1 border border-slate-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-accent-500 disabled:bg-slate-50 dark:bg-warm-900" />
 <span class="text-sm text-slate-500 dark:text-slate-400">minutes</span>
 </div>
 <p class="text-[11px] text-slate-400 dark:text-slate-500">Manual + JIT (first-login auto-provision via the LDAP login strategy) always work regardless of schedule.</p>
 </fieldset>

 <!-- On missing -->
 <div>
 <label for="on-missing" class="block text-xs text-slate-500 dark:text-slate-400 mb-1">When a user disappears from the directory</label>
 <select id="on-missing" bind:value={onMissing}
 class="px-3 py-1.5 border border-slate-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-accent-500">
 <option value="disable">Disable the local user (reversible)</option>
 <option value="ignore">Leave the local user alone</option>
 </select>
 <p class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">Only users provisioned by this source are affected. Local users and users from other sources are never touched.</p>
 </div>

 <!-- Test result panel -->
 {#if testResult}
 <div class="border border-slate-200 dark:border-warm-700 rounded-md p-3 bg-slate-50 dark:bg-warm-900 text-[11px]">
 <div class="font-medium text-slate-700 dark:text-slate-200 mb-1.5">Test results · {testResult.total} entries returned</div>
 {#if testResult.entries.length === 0}
 <div class="text-slate-500 dark:text-slate-400 italic">No matches. Verify Base DN + filter.</div>
 {:else}
 <ul class="space-y-2">
 {#each testResult.entries as e}
 <li class="border border-slate-200 dark:border-warm-700 rounded p-2 bg-white dark:bg-warm-900">
 <div class="font-mono text-slate-600 dark:text-slate-300 truncate" title={e.dn}>{e.dn}</div>
 <div class="text-slate-500 dark:text-slate-400 mt-1 space-y-0.5">
 {#each Object.entries(e.attributes) as [k, vals]}
 <div class="font-mono"><span class="text-slate-400 dark:text-slate-500">{k}:</span> {vals.join(', ')}</div>
 {/each}
 </div>
 </li>
 {/each}
 ...
 <div class="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-warm-700">
 ...
 class="px-4 py-1.5 bg-white dark:bg-warm-900 border border-slate-300 text-slate-600 dark:text-slate-300 text-sm rounded-md hover:bg-slate-50 dark:bg-warm-900 transition-colors cursor-pointer">
 Cancel
 </button>
 {#if editingIndex !== -1}
 <button onclick={runTest}
 class="ml-auto px-3 py-1.5 bg-amber-50 text-amber-700 text-sm rounded-md hover:bg-amber-100 transition-colors flex items-center gap-1.5 cursor-pointer">
