<script lang="ts">
    ...
    let testResult = $state<{
        source_id: string;
        total: number;
        entries: { dn: string; attributes: Record<string, string[]> }[];
    } | null>(null);
    ...
    function startEdit(idx: number) {
        ...
        groupRows = Object.entries(ldap.group_permissions ?? {}).map(
            ([group, ids]) => ({ group, permissionIds: [...ids] }),
        );
    ...
    function buildSourceFromForm(): SyncSource {
        ...
        return {
            ...
            schedule: {
                mode: scheduleMode,
                interval_minutes:
                    scheduleMode === "interval" ? scheduleMinutes : undefined,
            },
    ...
    async function saveCurrent() {
        ...
        if (!/^[a-z0-9-_]+$/.test(formId.trim())) {
            addToast(
                'Source ID may only contain lowercase letters, digits, "-" and "_"',
                "alert",
            );
            return;
        }
        ...
        try {
            ...
            if (editingIndex === -1) {
                if (next.some((s) => s.id === built.id)) {
                    addToast(
                        `Source ID "${built.id}" is already in use`,
                        "alert",
                    );
                    saving = false;
                    return;
...
<div>
    <div class="flex items-center justify-between mb-3">
        <div>
            <h2
                class="text-base font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2"
            >
                <Users size={16} /> User Sync
            </h2>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Provision and refresh users from external directories. Each
                source owns its synced users + group-mapped permissions; admin
    ...
    {#if editingIndex === null}
        ...
        {#if sources.length === 0}
            <div
                class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-8 text-center text-sm text-slate-500 dark:text-slate-400"
            >
        ...
        {:else}
            <div class="space-y-2">
                {#each sources as src, idx (src.id)}
                    ...
                    <div
                        class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-3"
                    >
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <span
                                        class="font-medium text-slate-800 dark:text-slate-100 text-sm"
                                        >{src.name}</span
                                    >
                                    <code
                                        class="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-warm-900 px-1.5 py-0.5 rounded"
                                        >{src.id}</code
                                    >
                                    <span
 ...
 {src.enabled
                                    ...
                                    <span
                                        ...
                                        >{status?.schedule_human ??
                                            "manual"}</span
                                    >
                                </div>
                                <div
                                    class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 truncate"
                                >
                                    {src.ldap?.address ?? ""} · {src.ldap
                                        ?.user_base_dn ?? ""}
                                </div>
                                {#if status?.last}
                                    <div
                                    ...
                                    >
                                        ...
                                        <span
                                            ...
                                            >{status.last.found}</span
                                        >
                                        · Created
                                        <span class="text-green-700 font-medium"
                                            >{status.last.created}</span
                                        >
                                        · Updated
                                        <span class="text-brand-700 font-medium"
                                            >{status.last.updated}</span
                                        >
                                        · Disabled
                                        <span class="text-amber-700 font-medium"
                                            >{status.last.disabled}</span
                                        >
                                        {#if status.last.errors?.length}
                                            · <span
                                                class="text-red-700 font-medium"
                                                >{status.last.errors.length} errors</span
                                            >
                                        {/if}
                                    </div>
                                {/if}
                            ...
                            <div class="flex items-center gap-1 shrink-0">
                                <button
                                    ...
                                    disabled={runningSourceId === src.id}
                                ...
                                >
                                    <RefreshCw
                                        size={11}
                                        class={runningSourceId === src.id
                                            ? "animate-spin"
                                            : ""}
                                    />
                                    {runningSourceId === src.id
                                        ? "Running…"
                                        : "Sync now"}
                                </button>
                                <button
                                    onclick={() => startEdit(idx)}
                                    class="px-2 py-1 text-[11px] bg-slate-100 dark:bg-warm-900 text-slate-700 dark:text-slate-200 rounded hover:bg-slate-200 dark:hover:bg-warm-700 transition-colors"
                                    >Edit</button
                                >
                                {#if confirmDeleteIdx === idx}
                                    <button
                                        onclick={() => deleteSource(idx)}
                                        class="px-2 py-1 text-[11px] bg-red-500 text-white rounded hover:bg-red-600 transition-colors"
                                        >Confirm</button
                                    >
                                    <button
                                        onclick={() => {
                                        ...
                                        class="px-2 py-1 text-[11px] bg-slate-200 dark:bg-warm-900 text-slate-600 dark:text-slate-300 rounded hover:bg-slate-300 dark:hover:bg-warm-600 transition-colors"
                                        >Cancel</button
                                    >
                                {:else}
                                    <button
                                        ...
                                        class="p-1 text-slate-400 dark:text-slate-500 hover:text-red-500 hover:bg-red-50 rounded transition-colors"
                                        title="Delete source"
                                    >
                                        <Trash2 size={14} />
                                    </button>
    ...
    {:else}
        <!-- ── Editor view ── -->
        <div
            class="bg-white dark:bg-warm-900 rounded-lg border border-slate-200 dark:border-warm-700 p-4 space-y-4"
        >
            <h3
            ...
            >
                {editingIndex === -1
                    ? "New sync source"
                    : `Edit "${sources[editingIndex]?.name}"`}
            </h3>
            ...
            <div class="grid grid-cols-2 gap-3">
                <div>
                    ...
                    >
                        Stable handle. Used internally as the user_identities
                        provider; cannot be changed after save.
                    </p>
                </div>
                <div>
                    <label
            ...
            <!-- Connection -->
            <fieldset
                class="border border-slate-200 dark:border-warm-700 rounded-md p-3 space-y-3"
            >
                <legend
                    class="text-xs font-medium text-slate-600 dark:text-slate-300 px-1"
                    >LDAP connection</legend
                >
                <div class="grid grid-cols-2 gap-3">
                    <div class="col-span-2">
                        ...
                        <p
                            class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5"
                        >
                            Bare host:port works too — TLS toggle below picks
                            the scheme.
                        </p>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button
                                type="button"
                                onclick={() => {
                                    showBindPassword = !showBindPassword;
                                }}
            ...
            >
                ...
                <p class="text-[11px] text-slate-500 dark:text-slate-400 -mt-1">
                    Each field is the LDAP attribute name to read for that pika
                    user field. Leave blank to skip.
                </p>
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <input
                            type="text"
                            bind:value={attrUsername}
                            placeholder="required · e.g. uid, sAMAccountName"
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            bind:value={attrSubject}
                            placeholder="stable ID; defaults to Username · e.g. entryUUID, objectGUID"
                            class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500"
                        />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input
                            type="text"
                            bind:value={attrDisplayName}
                            placeholder="e.g. displayName, gecos, cn"
                            class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500"
                        />
                    </div>
                    <div>
                        ...
                        <input
                            type="text"
                            bind:value={attrGivenName}
                            placeholder="optional · e.g. givenName"
                            class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500"
                        />
                    </div>
                    <div>
                        ...
                        <input
                            type="text"
                            bind:value={attrSurname}
                            placeholder="optional · e.g. sn"
                            class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500"
                        />
                    </div>
                    <div class="col-span-2">
                        ...
                        <input
                            type="text"
                            bind:value={attrGroups}
                            placeholder="multi-valued · e.g. memberOf — used by Group → Permission map below"
                            class="w-full px-2 py-1 border border-slate-300 rounded text-[11px] font-mono focus:outline-none focus:ring-1 focus:ring-accent-500"
                        />
                    </div>
                </div>
            ...
            >
                ...
                {#if permissions.length === 0}
                    <div
                        class="text-[11px] text-slate-400 dark:text-slate-500 italic py-2"
                    >
                        No permissions defined yet. Create them under Users →
                        Permissions first.
                    </div>
                {/if}
                {#each groupRows as row, idx (idx)}
                    <div
                        class="border border-slate-200 dark:border-warm-700 rounded p-2 space-y-1.5 bg-slate-50 dark:bg-warm-900/50"
                    >
                        <div class="flex items-center gap-1">
                            <input
                                ...
                                value={row.group}
                                oninput={(e) =>
                                    updateGroupRow(idx, e.currentTarget.value)}
                        ...
                        {#if permissions.length > 0}
                            <div class="flex flex-wrap gap-1.5">
                                {#each permissions as perm (perm.id)}
                                    {@const checked =
                                        row.permissionIds.includes(perm.id)}
                                    <button
                                        type="button"
                                        onclick={() =>
                                            toggleGroupPermission(idx, perm.id)}
                                        class="text-[10px] px-2 py-0.5 rounded border transition-colors
 {checked
                                            ? 'bg-accent-50 border-accent-300 text-brand-700 font-medium'
                                            : 'bg-white dark:bg-warm-900 border-slate-200 dark:border-warm-700 text-slate-600 dark:text-slate-300 hover:border-slate-300'}"
                                    >
                                        {perm.name}
                                    </button>
                                {/each}
            ...
            >
                ...
                <p class="text-[11px] text-slate-400 dark:text-slate-500">
                    Manual + JIT (first-login auto-provision via the LDAP login
                    strategy) always work regardless of schedule.
                </p>
            </fieldset>
            ...
            <div>
                ...
                <p
                    class="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5"
                >
                    Only users provisioned by this source are affected. Local
                    users and users from other sources are never touched.
                </p>
            </div>
            ...
            {#if testResult}
                <div
                    class="border border-slate-200 dark:border-warm-700 rounded-md p-3 bg-slate-50 dark:bg-warm-900 text-[11px]"
                >
                    <div
                        class="font-medium text-slate-700 dark:text-slate-200 mb-1.5"
                    >
                        Test results · {testResult.total} entries returned
                    </div>
                    {#if testResult.entries.length === 0}
                        <div class="text-slate-500 dark:text-slate-400 italic">
                            No matches. Verify Base DN + filter.
                        </div>
                    {:else}
                        <ul class="space-y-2">
                            {#each testResult.entries as e}
                                <li
                                    class="border border-slate-200 dark:border-warm-700 rounded p-2 bg-white dark:bg-warm-900"
                                >
                                    <div
                                        class="font-mono text-slate-600 dark:text-slate-300 truncate"
                                        title={e.dn}
                                    >
                                        {e.dn}
                                    </div>
                                    <div
                                        class="text-slate-500 dark:text-slate-400 mt-1 space-y-0.5"
                                    >
            ...
            <div
                class="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-warm-700"
            >
                <button
                    onclick={saveCurrent}
                    disabled={saving}
