<script lang="ts">
 ...
 import { Key, Plus, Trash2, Edit3, Smartphone, Usb, Wifi, Globe, Check, X, ShieldCheck, ShieldAlert, Copy, RefreshCw, AlertTriangle } from 'lucide-svelte';
 ...
 async function loadTOTPStatus() {
  ...
  try {
   ...
   totpStatus = null;
   totpLoadError = err?.response?.data?.message ?? err?.message ?? 'Failed to load TOTP status';
  } finally {
   totpLoading = false;
  }
 }
 ...
 async function startTOTPEnrollment() {
  ...
  try {
   ...
   const msg = err?.response?.data?.message ?? err?.message ?? 'Could not start enrollment';
   addToast(msg, 'alert');
  } finally {
 ...
 async function disableTOTP() {
  ...
  try {
   ...
   const msg = err?.response?.data?.message ?? err?.message ?? 'Failed to disable TOTP';
   addToast(msg, 'alert');
  } finally {
   disableBusy = false;
  }
 ...
 async function regenerateRecoveryCodes() {
  ...
  try {
   ...
   const msg = err?.response?.data?.message ?? err?.message ?? 'Failed to regenerate codes';
   addToast(msg, 'alert');
  } finally {
   regenBusy = false;
  }
...
<div>
  ...
  {:else}
   ...
   >
    ...
    <details class="text-xs">
     <summary class="cursor-pointer text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 select-none">
      Advanced: device preference
     </summary>
     <div class="mt-2 flex items-center gap-2">
      <label for="passkey-attachment" class="text-xs text-slate-600 dark:text-slate-300 shrink-0">
       Authenticator:
      </label>
      <select
      ...
      >
       ...
       <option value="platform">This device only (Touch ID, Windows Hello)</option>
       <option value="cross-platform">Security key (USB / NFC / BLE)</option>
      </select>
     </div>
     <p class="mt-1 text-[10px] text-slate-400 dark:text-slate-500 leading-snug">
 ...
 <div class="mt-4 bg-white dark:bg-warm-800 border border-slate-200 dark:border-warm-700 rounded-lg p-5">
  <div class="flex items-center justify-between mb-3">
   ...
   {#if totpStatus?.enabled}
    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 rounded text-[10px] font-medium text-emerald-700 dark:text-emerald-300 uppercase tracking-wide">
     Enabled
    </span>
   {:else if totpStatus?.pending_enrollment}
    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded text-[10px] font-medium text-amber-700 dark:text-amber-300 uppercase tracking-wide">
     Pending
    </span>
   {/if}
  </div>
  ...
  <p class="text-xs text-slate-500 dark:text-slate-400 mb-4 leading-relaxed">
   ...
   1Password, Authy, Bitwarden, ...) as a second factor after your
   password. After it is enabled every login requires entering the
  ...
  {:else if enrollment}
   ...
   <div class="space-y-4">
    <div class="flex flex-col sm:flex-row gap-4 items-start">
     {#if qrDataURI}
      <img src={qrDataURI} alt="Scan this QR with your authenticator app" class="w-60 h-60 border border-slate-200 dark:border-warm-700 rounded-md bg-white p-2" />
     {/if}
     <div class="flex-1 min-w-0 text-sm text-slate-700 dark:text-slate-200 space-y-3">
      <div>
       <div class="font-medium mb-1">1. Scan the QR with your authenticator app</div>
      ...
      <details class="text-xs">
       <summary class="cursor-pointer text-slate-600 dark:text-slate-300 hover:text-slate-800 dark:hover:text-white">
        Can't scan? Enter the secret manually
       </summary>
       <div class="mt-2 p-2 bg-slate-50 dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md font-mono text-[11px] break-all flex items-center gap-1">
        ...
        <button
         ...
         onclick={() => copyToClipboard(enrollment!.secret_base32, 'Secret')}
         title="Copy secret"
        >
         <Copy size={12} />
        </button>
       ...
       <div class="mt-1 text-[10px] text-slate-400 dark:text-slate-500">
        Algorithm: {enrollment.algorithm} · Digits: {enrollment.digits} · Period: {enrollment.period}s
       </div>
      </details>
      <div>
       <div class="font-medium mb-1">2. Enter the 6-digit code your app shows</div>
       <div class="flex gap-2">
        <input
         ...
         placeholder="123456"
         onkeydown={(e) => { if (e.key === 'Enter') { e.preventDefault(); finishTOTPEnrollment(); } }}
  ...
  {:else if totpStatus?.enabled}
   ...
   <div class="space-y-3">
    <div class="text-xs text-slate-500 dark:text-slate-400">
     Enabled {totpStatus.created_at ? new Date(totpStatus.created_at).toLocaleString() : ''}
     · Recovery codes left: <span class="font-medium {totpStatus.recovery_codes_left < 3 ? 'text-vermilion-600 dark:text-vermilion-400' : 'text-slate-700 dark:text-slate-200'}">{totpStatus.recovery_codes_left}</span>
    </div>

    {#if totpStatus.recovery_codes_left < 3 && totpStatus.recovery_codes_left > 0}
     <div class="p-2 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-md text-xs text-amber-800 dark:text-amber-200 flex items-start gap-2">
      <AlertTriangle size={14} class="shrink-0 mt-0.5" />
      <span>You have very few recovery codes left. Regenerate to get a fresh set.</span>
     </div>
    {/if}
    {#if totpStatus.recovery_codes_left === 0}
     <div class="p-2 bg-vermilion-50 dark:bg-vermilion-900/30 border border-vermilion-200 dark:border-vermilion-800 rounded-md text-xs text-vermilion-800 dark:text-vermilion-200 flex items-start gap-2">
      <AlertTriangle size={14} class="shrink-0 mt-0.5" />
      <span>No recovery codes remaining — if you lose your authenticator you will be locked out. Regenerate now.</span>
     </div>
    {/if}
    ...
    <div class="flex flex-wrap gap-2">
     <button
      type="button"
      onclick={() => { showRegen = !showRegen; showDisable = false; }}
      class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-warm-700 hover:bg-slate-200 dark:hover:bg-warm-600 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-md cursor-pointer"
     >
      <RefreshCw size={14} />
     ...
     <button
      type="button"
      onclick={() => { showDisable = !showDisable; showRegen = false; }}
    ...
    {#if showRegen}
     <div class="p-3 bg-slate-50 dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md space-y-2">
      <p class="text-xs text-slate-600 dark:text-slate-300">
       Confirm your password to regenerate. The new set will invalidate every previous recovery code.
      </p>
      <div class="flex gap-2">
       <input
        type="password"
        ...
        onkeydown={(e) => { if (e.key === 'Enter') { e.preventDefault(); regenerateRecoveryCodes(); } }}
    ...
    {#if showDisable}
     <div class="p-3 bg-vermilion-50/50 dark:bg-vermilion-900/10 border border-vermilion-200 dark:border-vermilion-800 rounded-md space-y-2">
      <p class="text-xs text-vermilion-700 dark:text-vermilion-300">
       Confirm your password to disable. After this you sign in with password only.
      </p>
      <div class="flex gap-2">
       <input
        type="password"
        ...
        placeholder="Your password"
        autocomplete="current-password"
        onkeydown={(e) => { if (e.key === 'Enter') { e.preventDefault(); disableTOTP(); } }}
  ...
  {:else}
   ...
   {#if totpStatus?.pending_enrollment}
    <p class="mt-2 text-[11px] text-amber-700 dark:text-amber-400">
     Previous enrollment was abandoned. Clicking the button above starts a fresh one — the old QR is no longer valid.
    </p>
   {/if}
  {/if}
 </div>
 ...
 {#if lastRecoveryCodes.length > 0}
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
   <div class="bg-white dark:bg-warm-800 border border-slate-200 dark:border-warm-700 rounded-lg shadow-xl max-w-md w-full p-5 space-y-3">
    <div class="flex items-start gap-2">
     <AlertTriangle size={18} class="text-amber-600 dark:text-amber-400 mt-0.5 shrink-0" />
     <div>
      <h3 class="text-base font-semibold text-slate-800 dark:text-slate-100">Save your recovery codes</h3>
      <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
       Each code can be used <strong>once</strong> if you lose access to your authenticator.
       <strong>Save them now</strong> — pika will never show them again.
      </p>
     </div>
    </div>

    <div class="p-3 bg-slate-50 dark:bg-warm-900 border border-slate-200 dark:border-warm-700 rounded-md font-mono text-xs grid grid-cols-2 gap-y-1 gap-x-3">
    ...
    <div class="flex gap-2 justify-end">
     <button
      type="button"
      onclick={() => copyToClipboard(lastRecoveryCodes.join('\n'), 'Recovery codes')}
      class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-warm-700 hover:bg-slate-200 dark:hover:bg-warm-600 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-md cursor-pointer"
     >
      <Copy size={14} />
      Copy all
