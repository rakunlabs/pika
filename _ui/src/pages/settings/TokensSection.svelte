...
{#if createdTokenKey}
 ...
 <code class="flex-1 px-3 py-2 bg-white dark:bg-warm-900 border border-green-200 rounded text-xs font-mono text-green-900 overflow-hidden text-ellipsis">
...
<div>
 ...
 <div>
 <h2 class="text-lg font-semibold text-slate-800 dark:text-slate-100">Access Tokens</h2>
 <p class="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Tokens authenticate consumers accessing configs via the data API</p>
 </div>
 <button
 ...
 <h3 class="text-sm font-semibold text-slate-700 dark:text-slate-200 mb-4">Create New Token</h3>

 <div class="mb-4">
 <label for="token-name" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Name</label>
 <input
 id="token-name"
 type="text"
 ...
 <div class="mb-4">
 <label for="token-expiry" class="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1.5">Expires At (optional)</label>
 <input
 id="token-expiry"
 type="datetime-local"
 ...
 <span class="block text-xs font-medium text-slate-500 dark:text-slate-400">Scopes</span>
 <button
 ...
 <div class="flex items-start gap-2 mb-2 p-3 bg-slate-50 dark:bg-warm-900 rounded-md border border-slate-100 dark:border-warm-700">
 <div class="flex-1">
 <input
 type="text"
 ...
 <label class="flex items-center gap-1 text-xs text-slate-600 dark:text-slate-300 cursor-pointer">
 <input
 type="checkbox"
 ...
 <p class="text-sm text-slate-500 dark:text-slate-400">No access tokens yet</p>
 <p class="text-xs text-slate-400 dark:text-slate-500 mt-1">Create a token to allow consumers to access configurations</p>
 </div>
 {:else}
 <div class="space-y-2">
 ...
 <div class="flex items-center gap-2">
 <span class="text-sm font-medium text-slate-800 dark:text-slate-100">{token.name}</span>
 <span class="px-1.5 py-0.5 text-[10px] font-medium rounded
 {token.active ? 'bg-green-100 text-green-700' : 'bg-slate-100 dark:bg-warm-900 text-slate-500 dark:text-slate-400'}">
 {token.active ? 'Active' : 'Disabled'}
 </span>
 </div>
 <div class="flex gap-3 mt-1 flex-wrap">
 <span class="text-xs text-slate-400 dark:text-slate-500">Created: {formatDate(token.created_at)}</span>
 {#if token.created_by}
 <span class="text-xs text-slate-400 dark:text-slate-500">by: <span class="text-slate-600 dark:text-slate-300">{token.created_by}</span></span>
 {/if}
 {#if token.expires_at}
 <span class="text-xs text-slate-400 dark:text-slate-500">Expires: {formatDate(token.expires_at)}</span>
 {/if}
 </div>
 <div class="flex flex-wrap gap-1.5 mt-2">
 {#each token.scopes as scope}
 <span class="px-2 py-0.5 text-[10px] font-mono bg-slate-100 dark:bg-warm-900 text-slate-600 dark:text-slate-300 rounded">
 ...
 {token.active ? 'text-amber-600 bg-amber-50 hover:bg-amber-100' : 'text-green-600 bg-green-50 hover:bg-green-100'} cursor-pointer"
