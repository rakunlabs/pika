<script lang="ts">
  ...
  import { Lock, RotateCw, ShieldOff, ShieldCheck, KeyRound, Loader2, AlertTriangle } from 'lucide-svelte';
  ...
  onMount(async () => {
    try {
      ...
      if (status.initialized) {
        ...
        lockMinutes = Math.max(1, Math.round((account.session_lock_seconds ?? 900) / 60));
      ...
      addToast(e?.response?.data?.message ?? 'Failed to load vault status', 'alert');
    } finally {
      loading = false;
    }
  });
  ...
  async function doRotate() {
    ...
    try {
      ...
      addToast(e?.response?.data?.message ?? e?.message ?? 'Rotate failed', 'alert');
    } finally {
      busy = false;
    }
  }
...
<div>
  ...
  {#if !vaultEnabled}
    <div class="bg-slate-50 dark:bg-warm-800 border border-slate-200 dark:border-warm-700 rounded p-3 text-sm text-slate-600 dark:text-slate-300">
      The vault feature is disabled on this server.
    </div>
  {:else if loading}
    <div class="text-sm text-slate-400"><Loader2 size={14} class="inline animate-spin" /> Loading...</div>
  {:else if !status?.initialized}
    <div class="bg-blue-50 dark:bg-blue-950/30 border border-blue-300 dark:border-blue-700 rounded p-3 text-sm">
      You haven't set up a vault yet. <a href="#/vault" class="underline text-accent-700 dark:text-accent-400">Set up the vault</a> to start storing secrets.
    </div>
  {:else}
    <div class="space-y-6">
      <!-- Auto-lock -->
      <div class="bg-white dark:bg-warm-800 border border-slate-200 dark:border-warm-700 rounded p-4">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mb-3">
          ...
          Locking zeroizes the in-memory key; you'll need to re-enter your master password to read items again.
        </p>
      ...
      <div class="bg-white dark:bg-warm-800 border border-slate-200 dark:border-warm-700 rounded p-4">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mb-3">
          When this browser is trusted, unlocking only requires your master password —
          the Secret Key is stored locally, encrypted with that password (Argon2id).
          Three wrong unlock attempts automatically revoke trust.
        </p>
        {#if trustInfo}
          <div class="flex items-center justify-between gap-3 flex-wrap">
            <div class="flex items-start gap-2 text-xs">
              <ShieldCheck size={14} class="shrink-0 mt-0.5 text-green-600 dark:text-green-500" />
              <div>
      ...
      <div class="bg-white dark:bg-warm-800 border border-accent-300 dark:border-accent-700 border-l-4 border-l-accent-500 dark:border-l-accent-400 rounded p-4">
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mb-3">
          ...
          Use this when you want to mark a previous kit as superseded (the old printout still works because the Secret Key isn't rotated; a future Secret Key rotation is planned).
        </p>
      ...
      <div class="bg-red-50 dark:bg-red-950/40 border border-red-300 dark:border-red-800 rounded p-4">
        ...
        <div class="flex items-start gap-2 text-xs text-red-700 dark:text-red-300 mb-3">
          <AlertTriangle size={14} class="shrink-0 mt-0.5" />
          <p>
            Deletes <strong>every item</strong> and the account record. Cannot be undone.
            Use this if you've lost your master password and want to start over.
          </p>
        </div>
        {#if !showReset}
          <button onclick={() => (showReset = true)}
