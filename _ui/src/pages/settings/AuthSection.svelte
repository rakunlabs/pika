...
<div>
  <div class="mb-4">
    <h2 class="text-lg font-semibold text-slate-800">Authentication</h2>
  ...
  <!-- ── UI section ── -->
  <div class="mb-3 bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
    <button
      type="button"
      class="w-full flex items-center justify-between px-5 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
      ...
      <div class="px-5 pb-5 pt-1 space-y-3 border-t border-slate-100">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="number" bind:value={issuerRefreshTTL} placeholder="86400" class="w-full px-3 py-2 text-sm border border-slate-200 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10" />
          </div>
        </div>
        <div class="pt-2">
          <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
          ...
          <p class="text-[11px] text-slate-400 mt-1.5 leading-relaxed">
            <span class="font-medium text-slate-600">Off (default):</span>
            ...
            <br />
            <span class="font-medium text-slate-600">On:</span> the
  ...
  <div class="mb-3 bg-white border border-slate-200 rounded-lg shadow-sm overflow-hidden">
    ...
    {#if openSections.has('oauth2')}
      <div class="px-5 pb-5 pt-3 border-t border-slate-100 space-y-4">
        {#each oauth2Entries as entry, i}
          <div class="p-4 bg-slate-50 border border-slate-200 rounded-lg space-y-3">
            <div class="flex items-center justify-between mb-1">
              <span class="text-xs font-semibold text-slate-600">Provider #{i + 1}</span>
              <button type="button" onclick={() => removeOAuth2(i)} class="p-1 text-slate-400 hover:text-red-500 transition-colors">
                <Trash2 size={13} />
              </button>
            </div>
            <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div class="flex gap-2">
                ...
                <button type="button" class="px-3 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-colors" onclick={() => addOAuth2Scope(i)}>Add</button>
              </div>
                ...
                <div class="mt-2 flex flex-wrap gap-1.5">
                  {#each entry.scopes ?? [] as scope, si}
                    <span class="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 border border-blue-200 rounded text-xs font-mono text-blue-700">
                      {scope}
                      <button type="button" class="w-3.5 h-3.5 flex items-center justify-center bg-transparent border-none cursor-pointer text-blue-400 hover:text-red-500" onclick={() => removeOAuth2Scope(i, si)}>&times;</button>
                    </span>
                  {/each}
                </div>
              {/if}
        ...
        <button
          type="button"
          onclick={addOAuth2}
          class="flex items-center gap-1.5 px-3 py-2 text-sm text-blue-600 bg-blue-50 rounded-md hover:bg-blue-100 transition-colors"
        >
    ...
    {#if openSections.has('ldap')}
      <div class="px-5 pb-5 pt-1 space-y-3 border-t border-slate-100">
        ...
        <div>
          ...
          <p class="mt-0.5 text-[10px] text-slate-400">Leave blank to keep existing password.</p>
        </div>
      </div>
    {/if}
  </div>
        ...
        <div>
          ...
          <div class="flex gap-2">
            <input type="text" bind:value={headerTrustedProxyInput} placeholder="10.0.0.0/8"
              ...
              class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10" />
            <button type="button" class="px-3 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-colors" onclick={addTrustedProxy}>Add</button>
          </div>
            ...
            <div class="mt-2 flex flex-wrap gap-1.5">
              {#each headerTrustedProxies as proxy, i}
                <span class="inline-flex items-center gap-1 px-2 py-1 bg-slate-50 border border-slate-200 rounded text-xs font-mono text-slate-600">
                  {proxy}
                  <button type="button" class="w-3.5 h-3.5 flex items-center justify-center bg-transparent border-none cursor-pointer text-slate-400 hover:text-red-500" onclick={() => removeTrustedProxy(i)}>&times;</button>
                </span>
              {/each}
            </div>
          {/if}
        ...
        <div>
          ...
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10"
            />
            <button type="button" onclick={addRlTrustedProxy} class="px-3 py-2 text-sm bg-slate-100 hover:bg-slate-200 rounded-md cursor-pointer">
              <Plus size={14} />
            </button>
          </div>
          {#if rlTrustedProxies.length > 0}
            <div class="mt-2 space-y-1">
              {#each rlTrustedProxies as cidr, i}
                <div class="flex items-center justify-between gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-md text-xs font-mono">
                  <span>{cidr}</span>
                  <button type="button" onclick={() => removeRlTrustedProxy(i)} class="text-slate-400 hover:text-red-600 cursor-pointer">
                    <Trash2 size={12} />
                  </button>
                </div>
              {/each}
      ...
      <div class="px-5 pb-5 pt-1 border-t border-slate-100 space-y-5">
        ...
        <div class="mt-3">
          ...
          <p class="text-[11px] text-slate-400 mb-2">
            ...
            the <code class="px-1 py-0.5 bg-slate-100 rounded">sub</code>
          ...
          <div class="flex gap-2">
            <input type="text" bind:value={capSuperadminInput} placeholder="username or OIDC sub"
              ...
              class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10" />
            <button type="button" class="px-3 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-colors cursor-pointer" onclick={addSuperadmin}>Add</button>
          </div>
        ...
        <div class="pt-4 border-t border-slate-100">
          ...
          <p class="text-[11px] text-slate-400 mb-2">
            ...
            <code class="px-1 py-0.5 bg-slate-100 rounded">roles</code>
          ...
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10"
            />
            <button type="button" class="flex items-center gap-1 px-3 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-colors cursor-pointer" onclick={addRoleMapping}>
              <Plus size={13} />
              Add role
            </button>
          </div>
          {#if capRoleMappings.length === 0}
            <p class="mt-3 text-[11px] text-slate-400 italic">No role mappings configured. External identities (OAuth2, LDAP, Header) will get zero capabilities unless listed as superadmins.</p>
          {:else}
                ...
                <div class="p-3 bg-slate-50 border border-slate-200 rounded-md">
                  <div class="flex items-center justify-between mb-2 gap-2">
                    <input
                      ...
                      class="flex-1 px-2 py-1 text-xs font-mono bg-white border border-slate-200 rounded focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10"
                    />
                    <button type="button" class="p-1 text-slate-400 hover:text-red-500 transition-colors cursor-pointer" onclick={() => removeRoleMapping(rowIdx)} aria-label="Remove role mapping">
                      <Trash2 size={13} />
                    </button>
                  </div>
                    ...
                    {#each knownCapabilities as cap}
                      <label class="flex items-start gap-2 text-xs text-slate-700 cursor-pointer">
                        ...
                        <span class="leading-tight">
                          ...
                          <span class="block text-[10px] text-slate-400 font-mono">{cap.key}</span>
                        </span>
                      </label>
                    {/each}
                  </div>
        ...
        <div class="pt-4 border-t border-slate-100">
          ...
          <p class="text-[11px] text-slate-400 mb-2">
            ...
            <code class="px-1 py-0.5 bg-slate-100 rounded">scope</code>
          ...
          <div class="flex gap-2">
            <input
              ...
              class="flex-1 px-3 py-2 text-sm font-mono border border-slate-200 rounded-md focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10"
            />
            <button type="button" class="flex items-center gap-1 px-3 py-2 text-sm text-white bg-blue-500 rounded-md hover:bg-blue-600 transition-colors cursor-pointer" onclick={addScopeMapping}>
              <Plus size={13} />
              Add scope
            </button>
          </div>
                ...
                <div class="p-3 bg-slate-50 border border-slate-200 rounded-md">
                  <div class="flex items-center justify-between mb-2 gap-2">
                    <input
                      ...
                      class="flex-1 px-2 py-1 text-xs font-mono bg-white border border-slate-200 rounded focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/10"
                    />
                    <button type="button" class="p-1 text-slate-400 hover:text-red-500 transition-colors cursor-pointer" onclick={() => removeScopeMapping(rowIdx)} aria-label="Remove scope mapping">
                      <Trash2 size={13} />
                    </button>
                  </div>
                  <div class="grid grid-cols-2 gap-x-3 gap-y-1">
