  ...
  {#if activeTab}
    ...
    <div class="flex items-center justify-between px-3 py-1.5 bg-[#252526] border-b border-[#3c3c3c] text-xs text-gray-400 shrink-0">
      <div class="flex items-center gap-2 min-w-0">
        ...
        <span class="text-gray-500 overflow-hidden text-ellipsis whitespace-nowrap" title="{activeTab.mount}/{activeTab.path}">
          {activeTab.mount}/{activeTab.path}
        </span>
      </div>
      <div class="flex items-center gap-1.5 shrink-0">
        {#if activeTab.contentType}
          <span class="px-1.5 py-0.5 text-[10px] text-gray-500 bg-[#1e1e1e] rounded border border-[#3c3c3c]">
            {activeTab.contentType.split(';')[0]}
          </span>
        {/if}
        <span class="text-[11px] text-gray-500">{filesStore.formatSize(activeTab.size)}</span>
        <span class="px-2 py-1 text-[11px] text-emerald-500">Read-only</span>
        <button
          class="flex items-center gap-1 px-2 py-1 text-[11px] text-gray-400 bg-transparent border border-[#3c3c3c] rounded cursor-pointer transition-colors hover:text-white hover:border-gray-500 hover:bg-[#333]"
    ...
    <div class="flex-1 min-h-0 overflow-auto flex flex-col">
      {#if !activeTab.loaded}
        <!-- Loading state -->
        <div class="flex flex-col items-center justify-center flex-1 gap-3 text-gray-500">
      ...
      {:else if activeTab.viewerMode === 'audio'}
        ...
        <div class="flex flex-col items-center justify-center flex-1 gap-6 bg-[#1e1e1e]">
          ...
          <div class="text-center">
            ...
            <p class="text-[12px] text-gray-500">{filesStore.formatSize(activeTab.size)}</p>
          </div>
          <audio
            src={activeTab.rawUrl}
            controls
      ...
      {:else if activeTab.viewerMode === 'binary-placeholder'}
        ...
        <div class="flex flex-col items-center justify-center flex-1 gap-4 text-gray-400">
          ...
          <div class="text-center">
            ...
            <p class="text-[13px] text-gray-500 mb-1">{activeTab.name}</p>
            <p class="text-[12px] text-gray-600">{filesStore.formatSize(activeTab.size)}</p>
            {#if activeTab.contentType}
              <p class="text-[11px] text-gray-600 mt-0.5">{activeTab.contentType.split(';')[0]}</p>
            {/if}
          </div>
          ...
          {#if activeTab.tooLargeForHex}
            ...
            <button
              class="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm cursor-pointer transition-colors hover:bg-blue-500"
              onclick={handleDownload}
            >
              <Download size={16} />
              Download File
          ...
          {:else}
            ...
            <p class="text-[11px] text-gray-600 max-w-xs text-center">
