...
<div class="select-none">
  ...
  >
    {#if isFolder}
      ...
      <span class="flex items-center justify-center shrink-0 {isActive ? 'text-white' : node.type === 'mount' ? 'text-blue-500' : 'text-amber-500'}">
        {#if node.type === 'mount'}
          <HardDrive size={14} />
    ...
    {#if isRenaming}
      ...
      <input
        ...
        class="flex-1 px-1 py-0 text-[13px] bg-white text-slate-800 border border-blue-400 rounded outline-none min-w-0"
...
{#if contextMenu}
  ...
  <div
    role="menu"
    tabindex="-1"
    class="fixed z-50 min-w-48 py-1 bg-white border border-slate-200 rounded-lg shadow-xl text-[13px]"
    style="left: {contextMenu.x}px; top: {contextMenu.y}px;"
    onclick={(e) => e.stopPropagation()}
    onkeydown={(e) => e.stopPropagation()}
  >
    {#if isFolder && isWritable}
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        onclick={(e) => { e.stopPropagation(); handleUploadClick(); }}>
        <Upload size={14} class="text-slate-400" /> Upload Files...
      </button>
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        onclick={(e) => { e.stopPropagation(); handleCreateFolder(); }}>
        <FolderPlus size={14} class="text-slate-400" /> New Folder...
      </button>
      <div class="my-1 border-t border-slate-150"></div>
    {/if}
    ...
    <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
      ...
      Copy
      <span class="ml-auto text-[11px] text-slate-400">Ctrl+C</span>
    </button>

    {#if isWritable && node.type !== 'mount'}
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        onclick={handleCut}>
        <Scissors size={14} class="text-slate-400" />
        Cut
        <span class="ml-auto text-[11px] text-slate-400">Ctrl+X</span>
      </button>
    {/if}
    ...
    {#if isFolder && isWritable && hasClipboard}
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        onclick={handlePaste}>
        <ClipboardPaste size={14} class="text-slate-400" />
        Paste {filesStore.clipboard ? `(${filesStore.clipboard.nodes.length})` : ''}
        <span class="ml-auto text-[11px] text-slate-400">Ctrl+V</span>
      </button>
    {/if}
    ...
    {#if isWritable && node.type !== 'mount'}
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        ...
        Rename
        <span class="ml-auto text-[11px] text-slate-400">F2</span>
      </button>
    {/if}
    ...
    {#if node.type === 'file'}
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        ...
        <Files size={14} class="text-slate-400" /> Duplicate
      </button>
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        onclick={handleDownload}>
        <Download size={14} class="text-slate-400" /> Download
      </button>
    {/if}
    ...
    {#if isFolder}
      <button class="flex items-center gap-2.5 w-full px-3 py-1.5 text-left text-slate-700 bg-transparent border-none cursor-pointer hover:bg-slate-100"
        ...
        <RefreshCw size={14} class="text-slate-400" /> Refresh
      </button>
    {/if}
