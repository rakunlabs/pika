...
<nav
   class="flex items-center h-10 bg-warm-900 text-white border-b border-warm-700 px-4 shrink-0"
>
 ...
 on the right. -->
  ...
  subordinate to the user identity it sits next to. -->
         <span class="px-2 text-[10px] font-mono text-warm-300 select-text"
            >{info.version}</span
         >
      {/if}
      {#if identity}
         <span
            class="flex items-center gap-1.5 px-3 py-1.5 rounded text-xs font-medium bg-warm-700 text-warm-100"
         >
            <User size={14} />
