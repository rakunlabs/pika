    ...
    {:else}
      ...
      >
        ...
        >
          <AlertTriangle
            size={16}
            class="text-blue-700 dark:text-blue-300 shrink-0 mt-0.5"
          />
          <div class="text-blue-900 dark:text-blue-200">
            Every item — title, tags, URLs, passwords, notes — is encrypted in
            your browser before it reaches the server. Search and filtering
            happen locally after unlock. The server can only see the item type
            (Login / Card / …) and lifecycle flags (favorite / archive / trash)
        ...
        <!-- Set expectations BEFORE the form. The Secret Key surprise
           ...
           only credential. -->
        <div
          class="bg-amber-50 dark:bg-amber-950/30 border border-amber-300 dark:border-amber-700 rounded p-3 text-sm mb-6 flex gap-2"
        >
          <KeyRound
            size={16}
          ...
          <div class="text-amber-900 dark:text-amber-200 space-y-1">
            <p class="font-semibold">
              You'll get a Secret Key after this step.
            </p>
            <p>
              In addition to the master password you choose here, a one-time
              <strong>Secret Key</strong> will be generated in your browser.
              You'll need
              <em>both</em> to unlock the vault on another device.
            </p>
            <p>
              The Secret Key is shown only once on the next screen — you'll be
              able to
              <strong>copy, download, or print</strong> it. Save it somewhere safe
              before continuing. Without it, a lost master password means the vault
              cannot be recovered.
            </p>
          </div>
        </div>
        ...
        <form onsubmit={submit} class="space-y-4">
          <div>
            ...
            {#if password}
              <div class="mt-2 flex items-center gap-2">
                <div
                  class="flex-1 h-1.5 bg-slate-200 dark:bg-warm-800 rounded overflow-hidden"
                >
                  <div
                    class="h-full transition-all {strengthColors[
                      strength.label
                    ]}"
                ...
                <span
                  class="text-xs capitalize text-slate-500 dark:text-slate-400"
                >
          ...
          <div>
            ...
            >
              <option value="fast"
                >Fast — 32 MiB, 2 iters (older / mobile)</option
              >
              <option value="default"
                >Default — 64 MiB, 3 iters (recommended)</option
              >
              <option value="strong"
                >Strong — 128 MiB, 4 iters (modern desktop)</option
              >
            </select>
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
              Stronger means each unlock takes longer in the browser. The
              setting is persisted with the vault; you can rotate it later.
            </p>
          </div>
          ...
          <p class="text-xs text-center text-slate-500 dark:text-slate-400">
            ...
            clicking Continue — it cannot be retrieved later.
          </p>
        </form>
      </div>
    {/if}
