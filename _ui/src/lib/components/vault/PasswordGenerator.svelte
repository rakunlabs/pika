...
<div class="space-y-3">
  ...
  <div class="flex gap-1 text-xs">
    <button
      ...
      class="px-3 py-1.5 rounded {mode === 'password'
        ? 'bg-accent-600 text-white'
        : 'bg-slate-100 dark:bg-warm-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-warm-700'} cursor-pointer"
      >Password</button
    >
    <button
      type="button"
      onclick={() => (mode = "passphrase")}
      class="px-3 py-1.5 rounded {mode === 'passphrase'
        ? 'bg-accent-600 text-white'
        : 'bg-slate-100 dark:bg-warm-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-warm-700'} cursor-pointer"
      >Passphrase</button
    >
  ...
  <div class="flex items-center gap-2">
    <div
      class="flex-1 h-1 bg-slate-200 dark:bg-warm-800 rounded overflow-hidden"
    >
      <div
    ...
    <div class="space-y-2 text-sm">
      <label class="flex items-center justify-between gap-3">
        ...
        <input
          ...
          max="128"
          bind:value={length}
          class="w-20 px-2 py-1 rounded border border-slate-300 dark:border-warm-600 bg-white dark:bg-warm-900 text-slate-800 dark:text-slate-100"
        />
      </label>
      ...
      <label class="flex items-center gap-2 cursor-pointer">
        <input type="checkbox" bind:checked={excludeAmbiguous} /> Exclude look-alike
        characters (I, l, O, 0…)
      </label>
    </div>
  {:else}
    <div class="space-y-2 text-sm">
      ...
      <label class="flex items-center justify-between gap-3">
        ...
        <input
          ...
          class="w-20 px-2 py-1 rounded border border-slate-300 dark:border-warm-600 bg-white dark:bg-warm-900 text-slate-800 dark:text-slate-100 font-mono"
        />
      </label>
      <label class="flex items-center gap-2 cursor-pointer">
