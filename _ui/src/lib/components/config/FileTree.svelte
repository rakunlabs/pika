<script lang="ts">
  ...
  function toggleSearchMode() {
    configStore.setSearchMode(
      configStore.searchMode === "name" ? "all" : "name",
    );
  }
...
<div
  class="flex flex-col h-full bg-slate-50 dark:bg-warm-900 border-r border-slate-200 dark:border-warm-700 select-none"
>
  <!-- Header -->
  <div
 ...
 {configStore.searchMode === 'name'
  ...
  {#if isActive && (hasResults || configStore.isSearching)}
    <div class="flex-1 overflow-y-auto">
      <!-- Status bar -->
      <div
        class="flex items-center justify-between px-3 py-1.5 text-[11px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-warm-900 border-b border-slate-200 dark:border-warm-700"
      >
        <span>
          {configStore.searchResults.length} result{configStore.searchResults
            .length !== 1
            ? "s"
            : ""}
        </span>
      ...
      {#if nameResults.length > 0}
        <div
          class="px-3 py-1 text-[10px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider bg-slate-50 dark:bg-warm-900 border-b border-slate-100 dark:border-warm-700"
        >
          Config Names
        </div>
      ...
      {#if configStore.isSearching && !hasResults}
        <div
          class="flex items-center justify-center gap-2 py-8 text-slate-400 dark:text-slate-500 text-xs"
        >
  ...
  {:else if isActive && !configStore.isSearching && !hasResults}
    <div class="flex-1 flex items-start justify-center pt-8">
      <span class="text-xs text-slate-400 dark:text-slate-500"
        >No results for "{searchInput}"</span
      >
    </div>
  {:else}
    <!-- Tree -->
