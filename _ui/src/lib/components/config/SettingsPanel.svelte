<script lang="ts">
  ...
  const allVersions = $derived(
    activeTab ? [...activeTab.versions].reverse() : [],
  );
  const visibleVersions = $derived(
    versionsExpanded ? allVersions : allVersions.slice(0, 1),
  );
  ...
  async function startEditConstraint(
    version: number,
  ...
  async function saveConstraint(version: number) {
    if (!activeTab) return;
    try {
      await configStore.updateVersionConstraint(
        activeTab.id,
        version,
        editingConstraintValue.trim(),
      );
      editingConstraintVersion = null;
      editingConstraintValue = "";
    } catch {
  ...
  function diagramLabel(entry: InheritEntry): string {
    if (entry.resource)
      return entry.path ? `${entry.resource}:${entry.path}` : entry.resource;
  ...
  function handleGoTemplateChange(e: Event) {
    ...
    if (activeTab) {
      configStore.updateTabMeta(activeTab.id, {
        go_template: target.checked || undefined,
      });
    }
  }
  ...
  function removeInheritEntry(index: number) {
    ...
    configStore.updateTabMeta(activeTab.id, {
      inherits: current.length > 0 ? current : undefined,
    });
...
<div
  class="flex flex-col h-full bg-slate-50 dark:bg-warm-900 border-l border-slate-200 dark:border-warm-700"
>
  {#if activeTab}
    ...
    <div class="flex-1 overflow-y-auto p-3">
      ...
      <div class="mb-4">
        ...
        <label
          class="{cardClass} flex items-start gap-2 px-2.5 py-2 cursor-pointer hover:border-slate-300 dark:hover:border-warm-600 transition-colors"
        >
          <input
            type="checkbox"
          ...
          <span class="flex-1 min-w-0">
            <span
              class="block text-xs font-medium text-slate-700 dark:text-warm-100"
              >Go template</span
            >
            <span
              class="block mt-0.5 text-[10px] text-slate-400 dark:text-warm-400 leading-relaxed"
            >
              Runs mugo templates before parsing and inheritance. Default is
              off; shell, filesystem and env helpers are disabled server-side.
            </span>
          </span>
        </label>
      </div>
      ...
      <div class="mb-4">
          ...
          <div class="{cardClass} overflow-hidden">
            ...
            >
              {#each visibleVersions as ver}
                ...
                {@const isCurrent =
                  activeTab.version === ver.version ||
                  (activeTab.version === 0 &&
                    ver.version === activeTab.latestVersion)}
                ...
                >
                  <span class="flex items-center gap-2 min-w-0">
                    <button
                      ...
                      onclick={() =>
                        configStore.loadVersion(activeTab.id, ver.version)}
                      title="Load version {ver.version}">v{ver.version}</button
                    >
                    {#if isEditing}
                      ...
                      >
                        ...
                        <button
                          ...
                          title="Save constraint (Enter)">OK</button
                        >
                        <button
                  ...
                  <span
                    class="flex items-center gap-1.5 text-[10px] text-slate-400 dark:text-warm-400 pl-0.5"
                  >
                    {#if lastStatus?.author && lastStatus.author !== "system"}
                      <span class="text-slate-500 dark:text-warm-300"
                        >{lastStatus.author}</span
                      >
                      <span class="text-slate-300 dark:text-warm-500"
                        >&middot;</span
                      >
                    {/if}
        ...
        {:else}
          <p class="text-[11px] text-slate-400 dark:text-warm-400 italic">
            No versions yet
          </p>
        {/if}
      </div>
        ...
        <div class="mb-4">
          ...
          <div class="{cardClass} overflow-hidden">
            <div class="flex items-center gap-1.5 px-2.5 py-2">
              <code
                class="flex-1 text-[12px] font-mono text-slate-700 dark:text-warm-100 break-all select-all"
                >{dataEndpoint}</code
              >
              <button
            ...
            <div
              class="px-2.5 py-1.5 bg-slate-50 dark:bg-warm-900 border-t border-slate-100 dark:border-warm-700 text-[10px] text-slate-400 dark:text-warm-400 leading-relaxed"
            >
              <span class="text-slate-500 dark:text-warm-300"
                >Query params:</span
              >
              <code
                class="ml-1 px-1 py-0.5 bg-slate-100 dark:bg-warm-700 rounded text-slate-500 dark:text-warm-200"
                >?version=1.0.0</code
              >
              <code
                class="ml-1 px-1 py-0.5 bg-slate-100 dark:bg-warm-700 rounded text-slate-500 dark:text-warm-200"
      ...
      <div class="mb-4">
        <div class="flex items-center justify-between mb-2">
          ...
          >
            {#if showInheritance}<ChevronDown size={12} />{:else}<ChevronRight
                size={12}
              />{/if}
            <GitBranch size={12} />
        ...
        {#if showInheritance}
          ...
          {#if inheritEntries.length === 0}
            <p class="text-[11px] text-slate-400 dark:text-warm-400 italic">
              No inheritance. Add sources to merge configs from other files or
              external resources.
            </p>
          {:else}
            ...
            >
              <span class="flex items-center gap-1">
                {#if showMergeDiagram}<ChevronDown
                    size={10}
                  />{:else}<ChevronRight size={10} />{/if}
                <Layers size={10} />
            ...
            {#if showMergeDiagram}
              ...
              {@const head = truncated
                ? inheritEntries.slice(0, maxShown - 1)
                : inheritEntries}
              {@const tail = truncated ? inheritEntries[total - 1] : null}
              {@const hiddenCount = truncated ? total - maxShown : 0}
              <div
                class="mb-2 px-2 py-2 rounded bg-slate-50 dark:bg-warm-900 border border-slate-200 dark:border-warm-700"
              >
                <!-- Topmost layer: the current file. Always wins; visually
                     wider/stronger than the inherited layers below. -->
                <div class="relative">
                  <div
                    class="flex items-center gap-1.5 px-2 py-1 rounded border border-accent-300 dark:border-accent-700 bg-accent-50 dark:bg-accent-900/40 text-[10px]"
                  >
                    <FileText
                      size={10}
                      class="text-accent-700 dark:text-accent-300 shrink-0"
                    />
                    <span
                      class="font-mono text-accent-700 dark:text-accent-300 truncate flex-1"
                      title={activeTab.path}
                    >
                      {activeTab.path}
                    </span>
                    <span
                      class="text-[9px] font-semibold uppercase tracking-wide text-accent-600 dark:text-accent-300 shrink-0"
                      >wins</span
                    >
                  </div>
                </div>
                ...
                {#snippet layer(
                  ...
                  >
                    <div
                      class="flex items-center gap-1.5 px-2 py-1 rounded border {kindClass} text-[10px]"
                    >
                      <span
                        class="font-mono text-[8px] uppercase tracking-wider opacity-70 w-7 shrink-0"
                        >{kind}</span
                      >
                      <span
                        class="font-mono truncate flex-1"
                        title={diagramLabel(entry)}>{diagramLabel(entry)}</span
                      >
                      {#if isBase}
                        <span
                          class="text-[9px] font-semibold uppercase tracking-wide opacity-70 shrink-0"
                          >base</span
                        >
                      {/if}
                    </div>
                ...
                {#if truncated}
                  <div
                    class="relative -mt-px"
                    style="margin-left: {(maxShown - 1) * 8}px"
                  >
                    <div
                      class="px-2 py-0.5 text-center text-[9px] text-slate-400 dark:text-warm-400 italic border border-dashed border-slate-200 dark:border-warm-700 rounded bg-white/50 dark:bg-warm-800/50"
                    >
                      +{hiddenCount} more
                    </div>
                  </div>
                ...
                <p
                  class="mt-2 text-[9px] text-slate-400 dark:text-warm-400 leading-snug"
                >
                  Higher layer wins on key conflicts. Reorder with the ↑↓
                  buttons below.
                </p>
              </div>
            {:else}
              <p class="text-[10px] text-slate-400 dark:text-warm-400 mb-1.5">
                Order = precedence among sources (top wins). Your local file
                always overrides.
              </p>
            {/if}
            ...
            <div class="space-y-1.5">
              {#each inheritEntries as entry, i (i)}
                <div class="{cardClass} overflow-hidden">
                  <div
                    class="flex items-start justify-between px-2.5 py-2 gap-2"
                  >
                    <div class="flex-1 min-w-0">
                      {#if entry.resource}
                        <div class="flex items-center gap-1 text-xs">
                          <span
                            class="px-1 py-0.5 bg-purple-50 dark:bg-purple-900/40 text-purple-600 dark:text-purple-200 rounded text-[10px] font-medium shrink-0"
                            >ext</span
                          >
                          <span
                            class="font-mono text-accent-700 dark:text-accent-300 truncate"
                            title="{entry.resource}:{entry.path || ''}"
                            >{entry.resource}</span
                          >
                        </div>
                        {#if entry.path}
                          <div
                            class="text-[10px] text-slate-400 dark:text-warm-400 mt-0.5"
                          >
                            path: <span
                              class="font-mono text-slate-500 dark:text-warm-200"
                              >{entry.path}</span
                            >
                          </div>
                        {/if}
                      {:else}
                        <div
                          class="text-xs font-mono text-accent-700 dark:text-accent-300 truncate"
                          title={entry.source}
                        >
                          {entry.source}
                        </div>
                      ...
                      {#if entry.inject}
                        ...
                        >
                          inject: <span
                            class="font-mono text-emerald-600 dark:text-emerald-300"
                            >{entry.inject}</span
                          >
                        </div>
                      {/if}
      ...
      {#if !activeTab.variantKey}
        <div class="mb-4">
          <div class="flex items-center justify-between mb-2">
            <span
              class="flex items-center gap-1.5 text-[11px] font-semibold text-slate-500 dark:text-warm-300 uppercase tracking-wide"
            >
              <Layers size={12} />
              Variants
            </span>
          ...
          <p class="text-[11px] text-slate-400 dark:text-warm-400">
            Variants are independent configs accessed via <span
              class="font-mono">?variant=name</span
            >. Manage them in the file tree.
          </p>
        </div>
      {:else}
        <div class="mb-4">
          <span
            class="flex items-center gap-1.5 text-[11px] font-medium text-purple-600 dark:text-purple-300 mb-1"
          >
            <Layers size={12} />
            Variant: <span class="font-mono">@{activeTab.variantKey}</span>
          </span>
          <p class="text-[11px] text-slate-400 dark:text-warm-400">
            This is an independent variant of <span
              class="font-mono text-slate-500 dark:text-warm-200"
              >{activeTab.path}</span
            >
          </p>
      ...
      <div class="mb-2">
        <h4
          class="text-[11px] font-semibold text-slate-500 dark:text-warm-300 mb-2.5 uppercase tracking-wide"
        >
          File Information
        </h4>
