...
<div
  class="flex items-stretch bg-slate-100 dark:bg-warm-800 border-b border-slate-200 dark:border-warm-700 min-h-[35px] overflow-hidden"
>
  ...
  {:else}
    <div
      class="flex items-stretch overflow-x-auto overflow-y-hidden flex-1 scrollbar-thin scrollbar-track-slate-200 scrollbar-thumb-slate-400"
    >
  ...
  {isActive
        ...
        >
          ...
          {#if tab.isDirty}
            <span
              class="w-2 h-2 rounded-full bg-amber-500 shrink-0 group-hover:hidden"
            ></span>
          {/if}
          <button
