<script lang="ts">
  ...
  const maskExtensions = $derived([
    createMaskExtension(previewFormat),
    maskWatcher,
  ]);
    ...
    try {
      ...
      if (response.data.data) {
        try {
          ...
          const bytes = Uint8Array.from(binaryStr, (c: string) =>
            c.charCodeAt(0),
          );
...
{#if isOpen}
  ...
  >
    ...
    >
      ...
      <div
        class="flex flex-wrap items-center justify-between gap-x-3 gap-y-2 px-4 py-2 border-b border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-900"
      >
        <h2
          id="modal-title"
 ...
 {fullyMasked
      ...
      <div class="flex-1 min-h-0 flex flex-col min-h-[300px]">
        {#if isLoading}
          <div
            class="flex flex-col items-center justify-center h-full gap-3 text-slate-500 dark:text-slate-400"
          >
        ...
        {:else if error}
          <p
            class="px-4 py-2 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-xs border-b border-red-200 dark:border-red-800 shrink-0"
          >
            {error}
          </p>
          {#if renderedContent}
      ...
      <div
        class="flex items-center justify-between px-4 py-1.5 border-t border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-900"
      >
        ...
        <span class="text-[11px] text-slate-500 dark:text-slate-400">
          {#if activeTab?.meta.inherits?.length}
            ...
            {#each activeTab.meta.inherits as entry, i}
              {@const label = entry.mount
                ? entry.path
                  ? `${entry.mount}/${entry.path}`
                  : entry.mount
                : entry.resource
                  ? entry.path
              ...
              <span class="ml-1 inline-flex items-baseline gap-1">
                <span
                  class="px-1 py-px text-[9px] font-medium uppercase tracking-wider rounded
     {kind === 'mount'
                    ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200'
                    : kind === 'ext'
                      ? 'bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-200'
                      : 'bg-slate-100 text-slate-600 dark:bg-warm-700 dark:text-warm-200'}"
                  >{kind}</span
                >
                <strong class="text-brand-500 dark:text-brand-300 font-mono"
                  >{label}</strong
                >
                {#if entry.format}<span
                    class="text-slate-400 dark:text-warm-400"
                    >as {entry.format}</span
                  >{/if}
                {#if entry.inject}<span class="text-emerald-500"
                    >-&gt;{entry.inject}</span
                  >{/if}
              </span>{#if i < activeTab.meta.inherits.length - 1}<span
