<script lang="ts">
  ...
  import { X, GitBranch, Eye, Loader2, AlertTriangle, CheckCircle2 } from 'lucide-svelte';
  ...
  async function runPreview() {
    ...
    previewHasRun = false;
    try {
      const entry = await configStore.readExternalEntry(resourceField, pathField.trim());
      ...
      previewError = errObj?.response?.data?.message || errObj?.message || 'Preview failed';
    } finally {
  ...
  function buildEntry(): InheritEntry | null {
    ...
    const paths = pathsField.split(',').map(p => p.trim()).filter(p => p.length > 0);
  ...
  function handleBackdropMouseDown(e: MouseEvent) { mouseDownTarget = e.target; }
  function handleBackdropClick(e: MouseEvent) {
    if (e.target === e.currentTarget && mouseDownTarget === e.currentTarget) onClose();
  }
...
{#if isOpen}
  ...
  <div
    ...
    tabindex="-1"
  >
    <div class="bg-white dark:bg-warm-900 rounded-lg shadow-xl w-full max-w-[560px] max-h-[90vh] flex flex-col overflow-hidden">
      <!-- Header -->
      <div class="flex items-center justify-between px-5 py-4 border-b border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-900 shrink-0">
      ...
      <form onsubmit={handleSubmit} class="flex flex-col flex-1 overflow-hidden">
        <div class="p-5 overflow-y-auto flex-1 space-y-3">
          ...
          <fieldset>
            <legend class="block text-[11px] font-medium text-slate-500 dark:text-warm-300 mb-1.5 uppercase tracking-wide">Source Type</legend>
          ...
          {#if sourceType === 'internal'}
            <label class="block">
              <span class="block text-[11px] font-medium text-slate-500 dark:text-warm-300 mb-1.5 uppercase tracking-wide">Config Path</span>
              <input
                type="text"
          ...
          {:else}
            <label class="block">
              ...
              <select
                ...
                onchange={() => { pathField = ''; loadExternalPaths(resourceField); }}
              >
            ...
            {#if resourceField}
              <label class="block">
                <span class="block text-[11px] font-medium text-slate-500 dark:text-warm-300 mb-1.5 uppercase tracking-wide">Path</span>
                <input
                  type="text"
                  bind:value={pathField}
              ...
              {#if loadingPaths}
                <div class="text-[11px] text-slate-400 dark:text-warm-400">Loading paths...</div>
                ...
                <div class="max-h-40 overflow-y-auto {cardClass}">
                  {#each externalPathSuggestions as suggestion}
                    ...
                    <button
                      ...
                      onclick={() => {
                        if (isDir) {
                          pathField = (pathField ? pathField.replace(/\/?$/, '/') : '') + suggestion;
                          loadExternalPaths(resourceField, pathField);
                        } else {
                          const base = pathField.includes('/') ? pathField.replace(/[^/]*$/, '') : '';
          ...
          {#if sourceType !== 'internal'}
            <div>
              <span class="block text-[11px] font-medium text-slate-500 dark:text-warm-300 mb-1.5 uppercase tracking-wide">
                Decode As <span class="text-slate-400 dark:text-warm-400 normal-case font-normal">(optional)</span>
              </span>
              <div class="flex gap-1.5">
                ...
                {#if sourceType === 'external'}
                  <button
                    ...
                    onclick={runPreview}
                    disabled={previewLoading || !resourceField || !pathField.trim()}
                    title={!resourceField || !pathField.trim() ? 'Pick a resource and path first' : 'Read the entry and show what the backend returns'}
                  >
                    {#if previewLoading}
              ...
              <span class="block mt-1 text-[10px] text-slate-400 dark:text-warm-400 leading-snug">
                Use this when the secret value is a YAML / JSON / TOML document stored as a string (e.g. a Consul KV body). The backend will parse the string before merging.
              </span>
              ...
              <!-- Preview output. Three layered messages so the user can
                   ...
                   Decode As selection will actually do:
                     ...
                     3. Live decoded preview (client-side parse with the
                        selected format). -->
              {#if previewError}
                <div class="mt-2 p-2 rounded border border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/40 text-[11px] text-red-700 dark:text-red-300 flex items-start gap-1.5">
              ...
              {#if previewHasRun && !previewError}
                ...
                {#if previewWrappedString !== null}
                  <div class="mt-2 p-2 rounded border border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/40 text-[11px] text-amber-800 dark:text-amber-200 flex items-start gap-1.5">
                    <AlertTriangle size={12} class="shrink-0 mt-0.5" />
                    <span>
                      The backend returned a <code class="px-1 py-0.5 bg-amber-100 dark:bg-amber-900/40 rounded font-mono">{`{ "value": "..." }`}</code> wrapper — the entry is stored as a plain string. Pick a <strong>Decode As</strong> format to parse it before merge.
                    </span>
                  </div>
                {:else}
                  <div class="mt-2 p-2 rounded border border-emerald-300 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-950/40 text-[11px] text-emerald-800 dark:text-emerald-200 flex items-start gap-1.5">
                    <CheckCircle2 size={12} class="shrink-0 mt-0.5" />
                    <span>Returned a structured object — will merge as-is. Decode As is not needed.</span>
                  </div>
                {/if}
                ...
                <details class="mt-2" open>
                  <summary class="text-[10px] font-medium text-slate-500 dark:text-warm-300 uppercase tracking-wide cursor-pointer hover:text-slate-700 dark:hover:text-white">
                    Raw response
                  </summary>
                  <pre class="mt-1 max-h-40 overflow-auto px-2 py-1.5 text-[10px] font-mono leading-snug rounded border border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-900 text-slate-700 dark:text-warm-100">{JSON.stringify(previewRaw, null, 2)}</pre>
                </details>
                ...
                {#if previewWrappedString !== null && formatField}
                  <details class="mt-2" open>
                    ...
                    {#if previewDecodeError}
                      <div class="mt-1 p-2 rounded border border-red-300 dark:border-red-700 bg-red-50 dark:bg-red-950/40 text-[11px] text-red-700 dark:text-red-300 flex items-start gap-1.5">
                        <AlertTriangle size={12} class="shrink-0 mt-0.5" />
                        <span>{formatField.toUpperCase()} parse failed: {previewDecodeError}</span>
                      </div>
                    {:else}
                      <pre class="mt-1 max-h-40 overflow-auto px-2 py-1.5 text-[10px] font-mono leading-snug rounded border border-emerald-300 dark:border-emerald-700 bg-emerald-50/40 dark:bg-emerald-950/20 text-slate-700 dark:text-warm-100">{JSON.stringify(previewDecoded, null, 2)}</pre>
                    {/if}
                  </details>
                {/if}
              {/if}
          ...
          <label class="block">
            <span class="block text-[11px] font-medium text-slate-500 dark:text-warm-300 mb-1.5 uppercase tracking-wide">Include Paths <span class="text-slate-400 dark:text-warm-400 normal-case font-normal">(optional, comma-separated)</span></span>
            <input
              type="text"
            ...
            />
            <span class="block mt-1 text-[10px] text-slate-400 dark:text-warm-400 leading-snug">
              Leave empty to inherit the entire source. Filters are applied verbatim against the source's top-level keys after any Decode As step — list the names you actually want (e.g. <code class="font-mono">host</code>, <code class="font-mono">db.password</code>, <code class="font-mono">logging.*</code>). To pick a wrapper-style <code class="font-mono">value</code> key, write <code class="font-mono">value</code>.
            </span>
          </label>

          <!-- Inject target -->
          <label class="block">
            <span class="block text-[11px] font-medium text-slate-500 dark:text-warm-300 mb-1.5 uppercase tracking-wide">Inject At <span class="text-slate-400 dark:text-warm-400 normal-case font-normal">(optional)</span></span>
            <input
              type="text"
        ...
        <!-- Footer -->
        <div class="flex justify-end gap-2.5 px-5 py-4 border-t border-slate-200 dark:border-warm-700 bg-slate-50 dark:bg-warm-900 shrink-0">
          <button
            type="button"
