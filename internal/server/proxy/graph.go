...
// call site.
func (d CompileDeps) specFor(n *ProxyNode) (NodeSpec, *CompileError) {
	switch n.Type {
	case NodeTypeMiddleware:
		s, ok := d.Middlewares[n.Subtype]
		if !ok {
			return NodeSpec{}, &CompileError{NodeID: n.ID, Code: "unknown_middleware", Message: "unknown middleware subtype: " + n.Subtype}
		}
		return s, nil
	case NodeTypeHandler:
		s, ok := d.Handlers[n.Subtype]
		if !ok {
			return NodeSpec{}, &CompileError{NodeID: n.ID, Code: "unknown_handler", Message: "unknown handler subtype: " + n.Subtype}
		}
		return s, nil
	case NodeTypeSwitch:
...
func hashPipeline(srv ProxyServer) string {
	...
	for _, n := range nodes {
		fmt.Fprintf(h, "node=%s|type=%s|sub=%s|cfg=%s\n", n.ID, n.Type, n.Subtype, string(n.Config))
	}
